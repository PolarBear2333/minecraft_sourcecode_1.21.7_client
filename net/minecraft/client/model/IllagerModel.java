...
HeadedModel {
    ...
    public void setupAnim(S s) {
        boolean bl;
        super.setupAnim(s);
        this.head.yRot = ((IllagerRenderState)s).yRot * ((float)Math.PI / 180);
        this.head.xRot = ((IllagerRenderState)s).xRot * ((float)Math.PI / 180);
        if (((IllagerRenderState)s).isRiding) {
            ...
            float f = ((IllagerRenderState)s).walkAnimationSpeed;
            float f2 = ((IllagerRenderState)s).walkAnimationPos;
            this.rightArm.xRot = Mth.cos(f2 * 0.6662f + (float)Math.PI) * 2.0f * f * 0.5f;
            this.rightArm.yRot = 0.0f;
            this.rightArm.zRot = 0.0f;
            this.leftArm.xRot = Mth.cos(f2 * 0.6662f) * 2.0f * f * 0.5f;
            this.leftArm.yRot = 0.0f;
            this.leftArm.zRot = 0.0f;
            this.rightLeg.xRot = Mth.cos(f2 * 0.6662f) * 1.4f * f * 0.5f;
            this.rightLeg.yRot = 0.0f;
            this.rightLeg.zRot = 0.0f;
            this.leftLeg.xRot = Mth.cos(f2 * 0.6662f + (float)Math.PI) * 1.4f * f * 0.5f;
            this.leftLeg.yRot = 0.0f;
            this.leftLeg.zRot = 0.0f;
        }
        AbstractIllager.IllagerArmPose illagerArmPose = ((IllagerRenderState)s).armPose;
        if (illagerArmPose == AbstractIllager.IllagerArmPose.ATTACKING) {
            if (((ArmedEntityRenderState)s).getMainHandItem().isEmpty()) {
                AnimationUtils.animateZombieArms(this.leftArm, this.rightArm, true, ((IllagerRenderState)s).attackAnim, ((IllagerRenderState)s).ageInTicks);
            } else {
                AnimationUtils.swingWeaponDown(this.rightArm, this.leftArm, ((IllagerRenderState)s).mainArm, ((IllagerRenderState)s).attackAnim, ((IllagerRenderState)s).ageInTicks);
            }
            ...
            this.leftArm.z = 0.0f;
            this.leftArm.x = 5.0f;
            this.rightArm.xRot = Mth.cos(((IllagerRenderState)s).ageInTicks * 0.6662f) * 0.25f;
            this.leftArm.xRot = Mth.cos(((IllagerRenderState)s).ageInTicks * 0.6662f) * 0.25f;
            ...
            AnimationUtils.animateCrossbowCharge(this.rightArm, this.leftArm, ((IllagerRenderState)s).maxCrossbowChargeDuration, ((IllagerRenderState)s).ticksUsingItem, true);
        } else if (illagerArmPose == AbstractIllager.IllagerArmPose.CELEBRATING) {
            this.rightArm.z = 0.0f;
            this.rightArm.x = -5.0f;
            this.rightArm.xRot = Mth.cos(((IllagerRenderState)s).ageInTicks * 0.6662f) * 0.05f;
            this.rightArm.zRot = 2.670354f;
            this.rightArm.yRot = 0.0f;
            this.leftArm.z = 0.0f;
            this.leftArm.x = 5.0f;
            this.leftArm.xRot = Mth.cos(((IllagerRenderState)s).ageInTicks * 0.6662f) * 0.05f;
            this.leftArm.zRot = -2.3561945f;
            this.leftArm.yRot = 0.0f;
        }
