...
extends Sensor<PathfinderMob> {
    ...
    protected void doTick(ServerLevel serverLevel, PathfinderMob pathfinderMob) {
        ...
        List list = serverLevel.players().stream().filter(EntitySelector.NO_SPECTATORS).filter(serverPlayer -> targetingConditions.test(serverLevel, pathfinderMob, (LivingEntity)serverPlayer)).filter(this::playerHoldingTemptation).filter(serverPlayer -> !pathfinderMob.hasPassenger((Entity)serverPlayer)).sorted(Comparator.comparingDouble(pathfinderMob::distanceToSqr)).collect(Collectors.toList());
        if (!list.isEmpty()) {
