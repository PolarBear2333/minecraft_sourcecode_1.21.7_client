...
public class ItemModelGenerators {
    ...
    private ResourceLocation createFlatItemModel(Item item, String string, ModelTemplate modelTemplate) {
        return modelTemplate.create(ModelLocationUtils.getModelLocation(item, string), TextureMapping.layer0(TextureMapping.getItemTexture(item, string)), this.modelOutput);
    }
    ...
    private List<RangeSelectItemModel.Entry> createCompassModels(Item item) {
        ArrayList<RangeSelectItemModel.Entry> arrayList = new ArrayList<RangeSelectItemModel.Entry>();
        ItemModel.Unbaked unbaked = ItemModelUtils.plainModel(this.createFlatItemModel(item, "_16", ModelTemplates.FLAT_ITEM));
        arrayList.add(ItemModelUtils.override(unbaked, 0.0f));
        for (int i = 1; i < 32; ++i) {
            int n = Mth.positiveModulo(i - 16, 32);
            ItemModel.Unbaked unbaked2 = ItemModelUtils.plainModel(this.createFlatItemModel(item, String.format(Locale.ROOT, "_%02d", n), ModelTemplates.FLAT_ITEM));
            arrayList.add(ItemModelUtils.override(unbaked2, (float)i - 0.5f));
        }
        arrayList.add(ItemModelUtils.override(unbaked, 31.5f));
        return arrayList;
    }

    private void generateStandardCompassItem(Item item) {
        List<RangeSelectItemModel.Entry> list = this.createCompassModels(item);
        this.itemModelOutput.accept(item, ItemModelUtils.conditional(ItemModelUtils.hasComponent(DataComponents.LODESTONE_TRACKER), ItemModelUtils.rangeSelect((RangeSelectItemModelProperty)new CompassAngle(true, CompassAngleState.CompassTarget.LODESTONE), 32.0f, list), ItemModelUtils.inOverworld(ItemModelUtils.rangeSelect((RangeSelectItemModelProperty)new CompassAngle(true, CompassAngleState.CompassTarget.SPAWN), 32.0f, list), ItemModelUtils.rangeSelect((RangeSelectItemModelProperty)new CompassAngle(true, CompassAngleState.CompassTarget.NONE), 32.0f, list))));
    }

    private void generateRecoveryCompassItem(Item item) {
        this.itemModelOutput.accept(item, ItemModelUtils.rangeSelect((RangeSelectItemModelProperty)new CompassAngle(true, CompassAngleState.CompassTarget.RECOVERY), 32.0f, this.createCompassModels(item)));
    }
    ...
    private void generateClockItem(Item item) {
        ...
        arrayList.add(ItemModelUtils.override(unbaked, 63.5f));
        this.itemModelOutput.accept(item, ItemModelUtils.inOverworld(ItemModelUtils.rangeSelect((RangeSelectItemModelProperty)new Time(true, Time.TimeSource.DAYTIME), 64.0f, arrayList), ItemModelUtils.rangeSelect((RangeSelectItemModelProperty)new Time(true, Time.TimeSource.RANDOM), 64.0f, arrayList)));
    }
    ...
    private void generateTrimmableItem(Item item, ResourceKey<EquipmentAsset> resourceKey, ResourceLocation resourceLocation, boolean bl) {
        ...
        for (TrimMaterialData trimMaterialData : TRIM_MATERIAL_MODELS) {
            ...
            arrayList.add(ItemModelUtils.when(trimMaterialData.materialKey, unbaked));
        }
        if (bl) {
            ...
            object = ItemModelUtils.tintedModel(resourceLocation2, new Dye(-6265536));
        } else {
            ModelTemplates.FLAT_ITEM.create(resourceLocation2, TextureMapping.layer0(resourceLocation3), this.modelOutput);
            object = ItemModelUtils.plainModel(resourceLocation2);
        }
        this.itemModelOutput.accept(item, ItemModelUtils.select(new TrimMaterialProperty(), (ItemModel.Unbaked)object, arrayList));
    }
    ...
    private void generateBundleModels(Item item) {
        ...
        ItemModel.Unbaked unbaked2 = ItemModelUtils.composite(ItemModelUtils.plainModel(resourceLocation), new BundleSelectedItemSpecialRenderer.Unbaked(), ItemModelUtils.plainModel(resourceLocation2));
    ...
    private void generateBow(Item item) {
        ...
        this.itemModelOutput.accept(item, ItemModelUtils.conditional(ItemModelUtils.isUsingItem(), ItemModelUtils.rangeSelect((RangeSelectItemModelProperty)new UseDuration(false), 0.05f, unbaked2, ItemModelUtils.override(unbaked3, 0.65f), ItemModelUtils.override(unbaked4, 0.9f)), unbaked));
    }
    ...
    private void generateCrossbow(Item item) {
        ...
        this.itemModelOutput.accept(item, ItemModelUtils.select(new Charge(), ItemModelUtils.conditional(ItemModelUtils.isUsingItem(), ItemModelUtils.rangeSelect((RangeSelectItemModelProperty)new CrossbowPull(), unbaked2, ItemModelUtils.override(unbaked3, 0.58f), ItemModelUtils.override(unbaked4, 1.0f)), unbaked), ItemModelUtils.when(CrossbowItem.ChargeType.ARROW, unbaked5), ItemModelUtils.when(CrossbowItem.ChargeType.ROCKET, unbaked6)));
    }

    private void generateBooleanDispatch(Item item, ConditionalItemModelProperty conditionalItemModelProperty, ItemModel.Unbaked unbaked, ItemModel.Unbaked unbaked2) {
    ...
    private void generateBrush(Item item) {
        ...
        this.itemModelOutput.accept(item, ItemModelUtils.rangeSelect((RangeSelectItemModelProperty)new UseCycle(10.0f), 0.1f, unbaked, ItemModelUtils.override(unbaked2, 0.25f), ItemModelUtils.override(unbaked3, 0.5f), ItemModelUtils.override(unbaked4, 0.75f)));
    }
    ...
    private static ItemModel.Unbaked createFlatModelDispatch(ItemModel.Unbaked unbaked, ItemModel.Unbaked unbaked2) {
        return ItemModelUtils.select(new DisplayContext(), unbaked2, ItemModelUtils.when(List.of(ItemDisplayContext.GUI, ItemDisplayContext.GROUND, ItemDisplayContext.FIXED), unbaked));
    }
    ...
    private void generateSpyglass(Item item) {
        ...
        this.itemModelOutput.accept(item, ItemModelGenerators.createFlatModelDispatch(unbaked, unbaked2));
    }
    ...
    private void generateTrident(Item item) {
        ...
        this.itemModelOutput.accept(item, ItemModelGenerators.createFlatModelDispatch(unbaked, unbaked4));
    }
    ...
    private void generatePotion(Item item) {
        ResourceLocation resourceLocation = this.generateLayeredItem(item, ModelLocationUtils.decorateItemModelLocation("potion_overlay"), ModelLocationUtils.getModelLocation(item));
        this.addPotionTint(item, resourceLocation);
    }

    private void generateTippedArrow(Item item) {
        ResourceLocation resourceLocation = this.generateLayeredItem(item, ModelLocationUtils.getModelLocation(item, "_head"), ModelLocationUtils.getModelLocation(item, "_base"));
        this.addPotionTint(item, resourceLocation);
    }

    private void generateDyedItem(Item item, int n) {
        ResourceLocation resourceLocation = this.createFlatItemModel(item, ModelTemplates.FLAT_ITEM);
        this.itemModelOutput.accept(item, ItemModelUtils.tintedModel(resourceLocation, new Dye(n)));
    }
    ...
    private void generateTwoLayerDyedItem(Item item) {
        ...
        this.itemModelOutput.accept(item, ItemModelUtils.conditional(ItemModelUtils.hasComponent(DataComponents.DYED_COLOR), ItemModelUtils.tintedModel(resourceLocation4, BLANK_LAYER, new Dye(0)), ItemModelUtils.plainModel(resourceLocation3)));
    }
    ...
    extends Record {
        ...
        public TrimMaterialData(MaterialAssetGroup materialAssetGroup, ResourceKey<TrimMaterial> resourceKey) {
        ...
        public final boolean equals(Object object) {
            return (boolean)ObjectMethods.bootstrap("equals", new MethodHandle[]{TrimMaterialData.class, "assets;materialKey", "assets", "materialKey"}, this, object);
        }
