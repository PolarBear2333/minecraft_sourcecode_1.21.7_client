...
extends RealmsScreen {
    ...
    public RealmsSlotOptionsScreen(RealmsConfigureWorldScreen realmsConfigureWorldScreen, RealmsSlot realmsSlot, RealmsServer.WorldType worldType, int n) {
        ...
        this.defaultSlotName = realmsSlot.options.getDefaultSlotName(n);
        this.setWorldName(realmsSlot.options.getSlotName(n));
        if (worldType == RealmsServer.WorldType.NORMAL) {
            this.pvp = realmsSlot.options.pvp;
            this.spawnProtection = realmsSlot.options.spawnProtection;
            this.forceGameMode = realmsSlot.options.forceGameMode;
            this.spawnMonsters = realmsSlot.options.spawnMonsters;
            this.commandBlocks = realmsSlot.options.commandBlocks;
        } else {
            this.pvp = true;
    ...
    public void init() {
        ...
        if (this.worldType != RealmsServer.WorldType.NORMAL) {
            ...
            this.addLabel(new RealmsLabel((Component)object, this.width / 2, 26, -65536));
        }
        ...
        CycleButton<GameType> cycleButton3 = this.addRenderableWidget(CycleButton.builder(GameType::getShortDisplayName).withValues((Collection<GameType>)GAME_MODES).withInitialValue(this.gameMode).create(this.column1X, RealmsSlotOptionsScreen.row(3), this.columnWidth, 20, Component.translatable("selectWorld.gameMode"), (cycleButton, gameType) -> {
        ...
        CycleButton<Boolean> cycleButton4 = CycleButton.onOffBuilder(this.difficulty != Difficulty.PEACEFUL && this.spawnMonsters).create(n, RealmsSlotOptionsScreen.row(5), this.columnWidth, 20, Component.translatable("mco.configure.world.spawnMonsters"), this.confirmDangerousOption(mutableComponent, bl -> {
            this.spawnMonsters = bl;
        }));
        CycleButton<Difficulty> cycleButton5 = this.addRenderableWidget(CycleButton.builder(Difficulty::getDisplayName).withValues((Collection<Difficulty>)DIFFICULTIES).withInitialValue(this.difficulty).create(this.column1X, RealmsSlotOptionsScreen.row(5), this.columnWidth, 20, Component.translatable("options.difficulty"), (cycleButton2, difficulty) -> {
            this.difficulty = difficulty;
            if (this.worldType == RealmsServer.WorldType.NORMAL) {
                boolean bl;
                cycleButton.active = bl = this.difficulty != Difficulty.PEACEFUL;
                cycleButton4.setValue(bl && this.spawnMonsters);
            }
        }));
        this.addRenderableWidget(cycleButton4);
        CycleButton<Boolean> cycleButton6 = this.addRenderableWidget(CycleButton.onOffBuilder(this.forceGameMode).create(this.column1X, RealmsSlotOptionsScreen.row(7), this.columnWidth, 20, Component.translatable("mco.configure.world.forceGameMode"), (cycleButton, bl) -> {
            this.forceGameMode = bl;
        }));
        CycleButton<Boolean> cycleButton7 = this.addRenderableWidget(CycleButton.onOffBuilder(this.commandBlocks).create(n, RealmsSlotOptionsScreen.row(7), this.columnWidth, 20, Component.translatable("mco.configure.world.commandBlocks"), (cycleButton, bl) -> {
    ...
    private void saveSettings() {
        ...
        if (this.worldType == RealmsServer.WorldType.ADVENTUREMAP || this.worldType == RealmsServer.WorldType.EXPERIENCE || this.worldType == RealmsServer.WorldType.INSPIRATION) {
            this.parentScreen.saveSlotSettings(new RealmsSlot(this.slot.slotId, new RealmsWorldOptions(this.slot.options.pvp, this.slot.options.spawnMonsters, this.slot.options.spawnProtection, this.slot.options.commandBlocks, n, n2, this.slot.options.forceGameMode, this.worldName, this.slot.options.version, this.slot.options.compatibility), this.slot.settings));
        } else {
            boolean bl = this.worldType == RealmsServer.WorldType.NORMAL && this.difficulty != Difficulty.PEACEFUL && this.spawnMonsters;
            this.parentScreen.saveSlotSettings(new RealmsSlot(this.slot.slotId, new RealmsWorldOptions(this.pvp, bl, this.spawnProtection, this.commandBlocks, n, n2, this.forceGameMode, this.worldName, this.slot.options.version, this.slot.options.compatibility), this.slot.settings));
        }
    }
