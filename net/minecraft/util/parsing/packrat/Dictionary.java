...
public class Dictionary<S> {
    ...
    public void checkAllBound() {
        List<Atom> list = this.terms.entrySet().stream().filter(entry -> entry.getValue() == null).map(Map.Entry::getKey).toList();
        if (!list.isEmpty()) {
    ...
    record Reference<S, T>(Entry<S, T> ruleToParse, Atom<T> nameToStore) implements Term<S>
    {
        ...
        public boolean parse(ParseState<S> parseState, Scope scope, Control control) {
            T t = parseState.parse(this.ruleToParse);
            if (t == null) {
                return false;
            }
            scope.put(this.nameToStore, t);
            return true;
        }
    }
}
