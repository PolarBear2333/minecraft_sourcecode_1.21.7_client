...
implements SectionMesh {
    ...
    public boolean isEmpty(ChunkSectionLayer chunkSectionLayer) {
        return !this.buffers.containsKey((Object)chunkSectionLayer);
    }
    ...
    public void uploadMeshLayer(ChunkSectionLayer chunkSectionLayer, MeshData meshData, long l) {
        ...
        if (sectionBuffers != null) {
            if (sectionBuffers.getVertexBuffer().size() < meshData.vertexBuffer().remaining()) {
                ...
                sectionBuffers.setVertexBuffer(RenderSystem.getDevice().createBuffer(() -> "Section vertex buffer - layer: " + chunkSectionLayer.label() + "; cords: " + SectionPos.x(l) + ", " + SectionPos.y(l) + ", " + SectionPos.z(l), 40, meshData.vertexBuffer()));
            ...
            if (byteBuffer != null) {
                if (sectionBuffers.getIndexBuffer() == null || sectionBuffers.getIndexBuffer().size() < byteBuffer.remaining()) {
                    ...
                    sectionBuffers.setIndexBuffer(RenderSystem.getDevice().createBuffer(() -> "Section index buffer - layer: " + chunkSectionLayer.label() + "; cords: " + SectionPos.x(l) + ", " + SectionPos.y(l) + ", " + SectionPos.z(l), 72, byteBuffer));
            ...
            GpuBuffer gpuBuffer = RenderSystem.getDevice().createBuffer(() -> "Section vertex buffer - layer: " + chunkSectionLayer.label() + "; cords: " + SectionPos.x(l) + ", " + SectionPos.y(l) + ", " + SectionPos.z(l), 40, meshData.vertexBuffer());
            ByteBuffer byteBuffer = meshData.indexBuffer();
            GpuBuffer gpuBuffer2 = byteBuffer != null ? RenderSystem.getDevice().createBuffer(() -> "Section index buffer - layer: " + chunkSectionLayer.label() + "; cords: " + SectionPos.x(l) + ", " + SectionPos.y(l) + ", " + SectionPos.z(l), 72, byteBuffer) : null;
    ...
    public void uploadLayerIndexBuffer(ChunkSectionLayer chunkSectionLayer, ByteBufferBuilder.Result result, long l) {
        ...
        if (sectionBuffers.getIndexBuffer() == null) {
            sectionBuffers.setIndexBuffer(RenderSystem.getDevice().createBuffer(() -> "Section index buffer - layer: " + chunkSectionLayer.label() + "; cords: " + SectionPos.x(l) + ", " + SectionPos.y(l) + ", " + SectionPos.z(l), 72, result.byteBuffer()));
        } else {
            CommandEncoder commandEncoder = RenderSystem.getDevice().createCommandEncoder();
