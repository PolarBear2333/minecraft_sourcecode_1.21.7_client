...
extends Feature<PointedDripstoneConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<PointedDripstoneConfiguration> featurePlaceContext) {
        ...
        Optional<Direction> optional = PointedDripstoneFeature.getTipDirection(worldGenLevel, blockPos, randomSource);
        if (optional.isEmpty()) {
            return false;
        }
        BlockPos blockPos2 = blockPos.relative(optional.get().getOpposite());
        PointedDripstoneFeature.createPatchOfDripstoneBlocks(worldGenLevel, randomSource, blockPos2, pointedDripstoneConfiguration);
        int n = randomSource.nextFloat() < pointedDripstoneConfiguration.chanceOfTallerDripstone && DripstoneUtils.isEmptyOrWater(worldGenLevel.getBlockState(blockPos.relative(optional.get()))) ? 2 : 1;
        DripstoneUtils.growPointedDripstone(worldGenLevel, blockPos, optional.get(), n, false);
        return true;
    }
