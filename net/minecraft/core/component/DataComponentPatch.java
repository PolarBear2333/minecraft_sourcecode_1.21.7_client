...
public final class DataComponentPatch {
        ...
        return new StreamCodec<RegistryFriendlyByteBuf, DataComponentPatch>(){
            ...
            public void encode(RegistryFriendlyByteBuf registryFriendlyByteBuf, DataComponentPatch dataComponentPatch) {
                ...
                for (Reference2ObjectMap.Entry entry : Reference2ObjectMaps.fastIterable(dataComponentPatch.map)) {
                    ...
                    DataComponentType.STREAM_CODEC.encode(registryFriendlyByteBuf, object);
                }
            }
            ...
            private <T> void encodeComponent(RegistryFriendlyByteBuf registryFriendlyByteBuf, DataComponentType<T> dataComponentType, Object object) {
                codecGetter.apply(dataComponentType).encode(registryFriendlyByteBuf, object);
            }
    ...
    record PatchKey(DataComponentType<?> type, boolean removed) {
        public static final Codec<PatchKey> CODEC = Codec.STRING.flatXmap(string -> {
            ...
            if ((dataComponentType = BuiltInRegistries.DATA_COMPONENT_TYPE.getValue(resourceLocation = ResourceLocation.tryParse(string))) == null) {
            ...
            return DataResult.success((Object)new PatchKey(dataComponentType, bl));
        }, patchKey -> {
            ...
            return DataResult.success((Object)(patchKey.removed() ? DataComponentPatch.REMOVED_PREFIX + String.valueOf(resourceLocation) : resourceLocation.toString()));
        });
