...
public class WaypointCommand {
    public static void register(CommandDispatcher<CommandSourceStack> commandDispatcher, CommandBuildContext commandBuildContext) {
        commandDispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("waypoint").requires(Commands.hasPermission(2))).then(Commands.literal("list").executes(commandContext -> WaypointCommand.listWaypoints((CommandSourceStack)commandContext.getSource())))).then(Commands.literal("modify").then(((RequiredArgumentBuilder)Commands.argument("waypoint", EntityArgument.entity()).then(((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("color").then(Commands.argument("color", ColorArgument.color()).executes(commandContext -> WaypointCommand.setWaypointColor((CommandSourceStack)commandContext.getSource(), WaypointArgument.getWaypoint((CommandContext<CommandSourceStack>)commandContext, "waypoint"), ColorArgument.getColor((CommandContext<CommandSourceStack>)commandContext, "color"))))).then(Commands.literal("hex").then(Commands.argument("color", HexColorArgument.hexColor()).executes(commandContext -> WaypointCommand.setWaypointColor((CommandSourceStack)commandContext.getSource(), WaypointArgument.getWaypoint((CommandContext<CommandSourceStack>)commandContext, "waypoint"), HexColorArgument.getHexColor((CommandContext<CommandSourceStack>)commandContext, "color")))))).then(Commands.literal("reset").executes(commandContext -> WaypointCommand.resetWaypointColor((CommandSourceStack)commandContext.getSource(), WaypointArgument.getWaypoint((CommandContext<CommandSourceStack>)commandContext, "waypoint")))))).then(((LiteralArgumentBuilder)Commands.literal("style").then(Commands.literal("reset").executes(commandContext -> WaypointCommand.setWaypointStyle((CommandSourceStack)commandContext.getSource(), WaypointArgument.getWaypoint((CommandContext<CommandSourceStack>)commandContext, "waypoint"), WaypointStyleAssets.DEFAULT)))).then(Commands.literal("set").then(Commands.argument("style", ResourceLocationArgument.id()).executes(commandContext -> WaypointCommand.setWaypointStyle((CommandSourceStack)commandContext.getSource(), WaypointArgument.getWaypoint((CommandContext<CommandSourceStack>)commandContext, "waypoint"), ResourceKey.create(WaypointStyleAssets.ROOT_ID, ResourceLocationArgument.getId((CommandContext<CommandSourceStack>)commandContext, "style"))))))))));
    }
    ...
    private static int setWaypointColor(CommandSourceStack commandSourceStack, WaypointTransmitter waypointTransmitter, Integer n) {
        WaypointCommand.mutateIcon(commandSourceStack, waypointTransmitter, icon -> {
            icon.color = Optional.of(n);
        });
        commandSourceStack.sendSuccess(() -> Component.translatable("commands.waypoint.modify.color", Component.literal(String.format("%06X", ARGB.color(0, (int)n))).withColor(n)), false);
        return 0;
    }
