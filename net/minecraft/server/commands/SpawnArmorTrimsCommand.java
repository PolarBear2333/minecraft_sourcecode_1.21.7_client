...
public class SpawnArmorTrimsCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> commandDispatcher) {
        commandDispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("spawn_armor_trims").requires(Commands.hasPermission(2))).then(Commands.literal("*_lag_my_game").executes(commandContext -> SpawnArmorTrimsCommand.spawnAllArmorTrims((CommandSourceStack)commandContext.getSource(), ((CommandSourceStack)commandContext.getSource()).getPlayerOrException())))).then(Commands.argument("pattern", ResourceKeyArgument.key(Registries.TRIM_PATTERN)).executes(commandContext -> SpawnArmorTrimsCommand.spawnArmorTrim((CommandSourceStack)commandContext.getSource(), ((CommandSourceStack)commandContext.getSource()).getPlayerOrException(), ResourceKeyArgument.getRegistryKey((CommandContext<CommandSourceStack>)commandContext, "pattern", Registries.TRIM_PATTERN, ERROR_INVALID_PATTERN)))));
    }
    ...
    private static int spawnArmorTrims(CommandSourceStack commandSourceStack, Player player, Stream<Holder.Reference<TrimPattern>> stream) {
        ...
        List<Holder.Reference> list = stream.sorted(Comparator.comparing(reference -> TRIM_PATTERN_ORDER.applyAsInt(reference.key()))).toList();
        List<Holder.Reference> list2 = serverLevel.registryAccess().lookupOrThrow(Registries.TRIM_MATERIAL).listElements().sorted(Comparator.comparing(reference -> TRIM_MATERIAL_ORDER.applyAsInt(reference.key()))).toList();
        ...
        for (int i = 0; i < list2.size(); ++i) {
            Holder.Reference reference2 = list2.get(i);
            for (int j = 0; j < list.size(); ++j) {
                Holder.Reference reference3 = list.get(j);
                ArmorTrim armorTrim = new ArmorTrim(reference2, reference3);
                for (int k = 0; k < list3.size(); ++k) {
                    Holder.Reference<Item> reference4 = list3.get(k);
                    double d2 = (double)blockPos.getX() + 0.5 - (double)k * 3.0;
                    double d3 = (double)blockPos.getY() + 0.5 + (double)i * 3.0;
                    double d4 = (double)blockPos.getZ() + 0.5 + (double)(j * 10);
                    ArmorStand armorStand = new ArmorStand(serverLevel, d2, d3, d4);
                    armorStand.setYRot(180.0f);
                    armorStand.setNoGravity(true);
                    ItemStack itemStack = new ItemStack(reference4);
    ...
    private static List<Holder.Reference<Item>> findEquippableItemsWithAssets(HolderLookup<Item> holderLookup) {
        ArrayList<Holder.Reference<Item>> arrayList = new ArrayList<Holder.Reference<Item>>();
        holderLookup.listElements().forEach(reference -> {
            ...
            if (equippable != null && equippable.slot().getType() == EquipmentSlot.Type.HUMANOID_ARMOR && equippable.assetId().isPresent()) {
                arrayList.add((Holder.Reference<Item>)reference);
            }
        });
        return arrayList;
    }
