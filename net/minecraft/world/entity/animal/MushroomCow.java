...
implements Shearable {
    ...
    public InteractionResult mobInteract(Player player, InteractionHand interactionHand) {
        ...
        if (itemStack.is(Items.SHEARS) && this.readyForShearing()) {
            ...
            if (level instanceof ServerLevel) {
                ...
                itemStack.hurtAndBreak(1, (LivingEntity)player, MushroomCow.getSlotForHand(interactionHand));
            }
            return InteractionResult.SUCCESS;
        }
    ...
    protected <T> boolean applyImplicitComponent(DataComponentType<T> dataComponentType, T t) {
        if (dataComponentType == DataComponents.MOOSHROOM_VARIANT) {
            this.setVariant(MushroomCow.castComponentValue(DataComponents.MOOSHROOM_VARIANT, t));
            return true;
        }
    ...
    private Variant getOffspringVariant(MushroomCow mushroomCow) {
        Variant variant;
        Variant variant2 = this.getVariant();
        Variant variant3 = variant2 == (variant = mushroomCow.getVariant()) && this.random.nextInt(1024) == 0 ? (variant2 == Variant.BROWN ? Variant.RED : Variant.BROWN) : (this.random.nextBoolean() ? variant2 : variant);
        return variant3;
    }
    ...
    public static enum Variant implements StringRepresentable
    {
        ...
        private Variant(String string2, int n2, BlockState blockState) {
            this.type = string2;
            this.id = n2;
