...
public class GameTestMainUtil {
    ...
    public static void runGameTestServer(String[] stringArray, Consumer<String> consumer) throws Exception {
        Object object;
        parser.allowsUnrecognizedOptions();
        OptionSet optionSet = parser.parse(stringArray);
        ...
        if (optionSet.has(packs)) {
            object = (String)optionSet.valueOf(packs);
            GameTestMainUtil.copyPacks(string, (String)object);
        }
        object = LevelStorageSource.createDefault(Paths.get(string, new String[0])).createAccess(LEVEL_NAME);
        PackRepository packRepository = ServerPacksSource.createPackRepository((LevelStorageSource.LevelStorageAccess)object);
        MinecraftServer.spin(arg_0 -> GameTestMainUtil.lambda$runGameTestServer$0((LevelStorageSource.LevelStorageAccess)object, packRepository, optionSet, arg_0));
    }
    ...
    private static void copyPacks(String string, String string2) throws IOException {
        Path path;
        Path path2 = Paths.get(string, new String[0]).resolve(LEVEL_NAME).resolve("datapacks");
    ...
    private static /* synthetic */ GameTestServer lambda$runGameTestServer$0(LevelStorageSource.LevelStorageAccess levelStorageAccess, PackRepository packRepository, OptionSet optionSet, Thread thread) {
        GameTestServer gameTestServer = GameTestServer.create(thread, levelStorageAccess, packRepository, GameTestMainUtil.optionalFromOption(optionSet, tests), optionSet.has(verify));
