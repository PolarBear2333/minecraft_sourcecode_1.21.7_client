...
extends EntityModel<CreeperRenderState> {
    ...
    public void setupAnim(CreeperRenderState creeperRenderState) {
        ...
        float f2 = creeperRenderState.walkAnimationPos;
        this.rightHindLeg.xRot = Mth.cos(f2 * 0.6662f) * 1.4f * f;
        this.leftHindLeg.xRot = Mth.cos(f2 * 0.6662f + (float)Math.PI) * 1.4f * f;
        this.rightFrontLeg.xRot = Mth.cos(f2 * 0.6662f + (float)Math.PI) * 1.4f * f;
