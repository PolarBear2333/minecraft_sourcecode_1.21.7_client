...
extends FlowerBlock {
    ...
    private boolean tryChangingState(BlockState blockState, ServerLevel serverLevel, BlockPos blockPos, RandomSource randomSource) {
        ...
        if (CreakingHeartBlock.isNaturalNight(serverLevel) == this.type.open) {
            return false;
        }
        ...
        BlockPos.betweenClosed(blockPos.offset(-3, -2, -3), blockPos.offset(3, 2, 3)).forEach(blockPos2 -> {
            ...
            if (blockState2 == blockState) {
                ...
                int n = randomSource.nextIntBetweenInclusive((int)(d * 5.0), (int)(d * 10.0));
                serverLevel.scheduleTick((BlockPos)blockPos2, blockState.getBlock(), n);
            }
        });
        return true;
    }
    ...
    public static enum Type {
        ...
        private Type(boolean bl, Holder<MobEffect> holder, float f, SoundEvent soundEvent, SoundEvent soundEvent2, int n2) {
            this.open = bl;
            this.effect = holder;
            this.effectDuration = f;
            this.longSwitchSound = soundEvent;
            this.shortSwitchSound = soundEvent2;
            this.particleColor = n2;
        }
