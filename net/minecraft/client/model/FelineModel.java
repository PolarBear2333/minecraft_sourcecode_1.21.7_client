...
extends EntityModel<T> {
    ...
    public void setupAnim(T t) {
        super.setupAnim(t);
        float f = ((FelineRenderState)t).ageScale;
        if (((FelineRenderState)t).isCrouching) {
            this.body.y += 1.0f * f;
            this.head.y += 2.0f * f;
            this.tail1.y += 1.0f * f;
            this.tail2.y += -4.0f * f;
        ...
        } else if (((FelineRenderState)t).isSprinting) {
            this.tail2.y = this.tail1.y;
            this.tail2.z += 2.0f * f;
            this.tail1.xRot = 1.5707964f;
            this.tail2.xRot = 1.5707964f;
        }
        this.head.xRot = ((FelineRenderState)t).xRot * ((float)Math.PI / 180);
        this.head.yRot = ((FelineRenderState)t).yRot * ((float)Math.PI / 180);
        if (!((FelineRenderState)t).isSitting) {
            this.body.xRot = 1.5707964f;
            float f2 = ((FelineRenderState)t).walkAnimationSpeed;
            float f3 = ((FelineRenderState)t).walkAnimationPos;
            if (((FelineRenderState)t).isSprinting) {
                this.leftHindLeg.xRot = Mth.cos(f3 * 0.6662f) * f2;
                this.rightHindLeg.xRot = Mth.cos(f3 * 0.6662f + 0.3f) * f2;
                this.leftFrontLeg.xRot = Mth.cos(f3 * 0.6662f + (float)Math.PI + 0.3f) * f2;
                this.rightFrontLeg.xRot = Mth.cos(f3 * 0.6662f + (float)Math.PI) * f2;
                this.tail2.xRot = 1.7278761f + 0.31415927f * Mth.cos(f3) * f2;
            } else {
                this.leftHindLeg.xRot = Mth.cos(f3 * 0.6662f) * f2;
                this.rightHindLeg.xRot = Mth.cos(f3 * 0.6662f + (float)Math.PI) * f2;
                this.leftFrontLeg.xRot = Mth.cos(f3 * 0.6662f + (float)Math.PI) * f2;
                this.rightFrontLeg.xRot = Mth.cos(f3 * 0.6662f) * f2;
                this.tail2.xRot = !((FelineRenderState)t).isCrouching ? 1.7278761f + 0.7853982f * Mth.cos(f3) * f2 : 1.7278761f + 0.47123894f * Mth.cos(f3) * f2;
            }
        }
        if (((FelineRenderState)t).isSitting) {
        ...
        if (((FelineRenderState)t).lieDownAmount > 0.0f) {
            this.head.zRot = Mth.rotLerp(((FelineRenderState)t).lieDownAmount, this.head.zRot, -1.2707963f);
            this.head.yRot = Mth.rotLerp(((FelineRenderState)t).lieDownAmount, this.head.yRot, 1.2707963f);
            ...
            this.tail1.xRot = Mth.rotLerp(((FelineRenderState)t).lieDownAmountTail, this.tail1.xRot, 0.8f);
            this.tail2.xRot = Mth.rotLerp(((FelineRenderState)t).lieDownAmountTail, this.tail2.xRot, -0.4f);
        }
        if (((FelineRenderState)t).relaxStateOneAmount > 0.0f) {
            this.head.xRot = Mth.rotLerp(((FelineRenderState)t).relaxStateOneAmount, this.head.xRot, -0.58177644f);
        }
    }
}
