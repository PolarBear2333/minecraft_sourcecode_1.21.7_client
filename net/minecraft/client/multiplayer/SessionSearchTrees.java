...
public class SessionSearchTrees {
    ...
    public void updateRecipes(ClientRecipeBook clientRecipeBook, Level level) {
        this.register(RECIPE_COLLECTIONS, () -> {
            ...
            HolderLookup.RegistryLookup registryLookup = registryAccess.lookupOrThrow(Registries.ITEM);
            ...
            this.recipeSearch = CompletableFuture.supplyAsync(() -> SessionSearchTrees.lambda$updateRecipes$8(contextMap, tooltipContext, default_, (Registry)registryLookup, list), Util.backgroundExecutor());
    ...
    public void updateCreativeTooltips(HolderLookup.Provider provider, List<ItemStack> list) {
        this.register(CREATIVE_NAMES, () -> {
            ...
            this.creativeByNameSearch = CompletableFuture.supplyAsync(() -> new FullTextSearchTree<ItemStack>(itemStack -> SessionSearchTrees.getTooltipLines(Stream.of(itemStack), tooltipContext, default_), itemStack -> itemStack.getItemHolder().unwrapKey().map(ResourceKey::location).stream(), list), Util.backgroundExecutor());
            completableFuture.cancel(true);
        });
    }
    ...
    private static /* synthetic */ SearchTree lambda$updateRecipes$8(ContextMap contextMap, Item.TooltipContext tooltipContext, TooltipFlag tooltipFlag, Registry registry, List list) {
