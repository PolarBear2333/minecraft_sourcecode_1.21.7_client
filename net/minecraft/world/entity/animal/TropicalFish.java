...
extends AbstractSchoolingFish {
    ...
    private void setBaseColor(DyeColor dyeColor) {
        int n = this.getPackedVariant();
        Pattern pattern = TropicalFish.getPattern(n);
        DyeColor dyeColor2 = TropicalFish.getPatternColor(n);
        this.setPackedVariant(TropicalFish.packVariant(pattern, dyeColor, dyeColor2));
    }
    ...
    private void setPatternColor(DyeColor dyeColor) {
        int n = this.getPackedVariant();
        Pattern pattern = TropicalFish.getPattern(n);
        DyeColor dyeColor2 = TropicalFish.getBaseColor(n);
    ...
    protected <T> boolean applyImplicitComponent(DataComponentType<T> dataComponentType, T t) {
        if (dataComponentType == DataComponents.TROPICAL_FISH_PATTERN) {
            this.setPattern(TropicalFish.castComponentValue(DataComponents.TROPICAL_FISH_PATTERN, t));
            return true;
        }
        if (dataComponentType == DataComponents.TROPICAL_FISH_BASE_COLOR) {
            this.setBaseColor(TropicalFish.castComponentValue(DataComponents.TROPICAL_FISH_BASE_COLOR, t));
            return true;
        }
        if (dataComponentType == DataComponents.TROPICAL_FISH_PATTERN_COLOR) {
            this.setPatternColor(TropicalFish.castComponentValue(DataComponents.TROPICAL_FISH_PATTERN_COLOR, t));
            return true;
        }
    ...
    TooltipProvider
    {
        ...
        public void addToTooltip(Item.TooltipContext tooltipContext, Consumer<Component> consumer, TooltipFlag tooltipFlag, DataComponentGetter dataComponentGetter) {
            ...
            int n = COMMON_VARIANTS.indexOf(new Variant(this, dyeColor, dyeColor2));
            if (n != -1) {
                consumer.accept(Component.translatable(TropicalFish.getPredefinedName(n)).withStyle(chatFormattingArray));
                return;
            }
            consumer.accept(this.displayName.plainCopy().withStyle(chatFormattingArray));
