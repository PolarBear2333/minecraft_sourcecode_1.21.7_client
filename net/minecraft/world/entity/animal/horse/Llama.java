...
implements RangedAttackMob {
    ...
    protected <T> boolean applyImplicitComponent(DataComponentType<T> dataComponentType, T t) {
        if (dataComponentType == DataComponents.LLAMA_VARIANT) {
            this.setVariant(Llama.castComponentValue(DataComponents.LLAMA_VARIANT, t));
            return true;
        }
    ...
    private void spit(LivingEntity livingEntity) {
        ...
        if (level instanceof ServerLevel) {
            ...
            Projectile.spawnProjectileUsingShoot(llamaSpit, serverLevel, ItemStack.EMPTY, d, d2 + d4, d3, 1.5f, 10.0f);
        }
        if (!this.isSilent()) {
