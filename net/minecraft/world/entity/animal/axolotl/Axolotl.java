...
implements Bucketable {
    ...
    public void baseTick() {
        ...
        if (!this.isNoAi() && (level = this.level()) instanceof ServerLevel) {
            ...
            this.handleAirSupply(serverLevel, n);
        }
    ...
    protected void handleAirSupply(ServerLevel serverLevel, int n) {
    ...
    protected <T> boolean applyImplicitComponent(DataComponentType<T> dataComponentType, T t) {
        if (dataComponentType == DataComponents.AXOLOTL_VARIANT) {
            this.setVariant(Axolotl.castComponentValue(DataComponents.AXOLOTL_VARIANT, t));
            return true;
        }
    ...
    public boolean hurtServer(ServerLevel serverLevel, DamageSource damageSource, float f) {
        ...
        if (!this.isNoAi() && this.level().random.nextInt(3) == 0 && ((float)this.level().random.nextInt(3) < f || f2 / this.getMaxHealth() < 0.5f) && f < f2 && this.isInWater() && (damageSource.getEntity() != null || damageSource.getDirectEntity() != null) && !this.isPlayingDead()) {
    ...
    public void loadFromBucketTag(CompoundTag compoundTag) {
        ...
        compoundTag.getLong("HuntingCooldown").ifPresentOrElse(l -> this.getBrain().setMemoryWithExpiry(MemoryModuleType.HAS_HUNTING_COOLDOWN, true, compoundTag.getLongOr("HuntingCooldown", 0L)), () -> this.getBrain().setMemory(MemoryModuleType.HAS_HUNTING_COOLDOWN, Optional.empty()));
    }
