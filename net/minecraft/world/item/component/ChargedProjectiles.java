...
implements TooltipProvider {
    ...
    public void addToTooltip(Item.TooltipContext tooltipContext, Consumer<Component> consumer, TooltipFlag tooltipFlag, DataComponentGetter dataComponentGetter) {
        ...
        for (ItemStack itemStack2 : this.items) {
            ...
            ChargedProjectiles.addProjectileTooltip(tooltipContext, consumer, itemStack, n);
            itemStack = itemStack2;
            n = 1;
        }
        if (itemStack != null) {
    ...
    private static void addProjectileTooltip(Item.TooltipContext tooltipContext, Consumer<Component> consumer, ItemStack itemStack, int n) {
        if (n == 1) {
            consumer.accept(Component.translatable("item.minecraft.crossbow.projectile.single", itemStack.getDisplayName()));
        } else {
            consumer.accept(Component.translatable("item.minecraft.crossbow.projectile.multiple", n, itemStack.getDisplayName()));
        }
