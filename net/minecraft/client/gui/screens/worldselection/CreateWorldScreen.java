...
extends Screen {
    ...
    public static void openFresh(Minecraft minecraft, @Nullable Screen screen) {
        CreateWorldScreen.openFresh(minecraft, screen, (createWorldScreen, layeredRegistryAccess, primaryLevelData, path) -> createWorldScreen.createNewWorld(layeredRegistryAccess, primaryLevelData));
    }
    ...
    public static void openFresh(Minecraft minecraft, @Nullable Screen screen, CreateWorldCallback createWorldCallback) {
        ...
        CreateWorldScreen.openCreateWorldScreen(minecraft, screen, function, worldCreationContextMapper, WorldPresets.NORMAL, createWorldCallback);
    }
    ...
    public static void testWorld(Minecraft minecraft, @Nullable Screen screen) {
        ...
        CreateWorldScreen.openCreateWorldScreen(minecraft, screen, function, worldCreationContextMapper, WorldPresets.FLAT, (createWorldScreen, layeredRegistryAccess, primaryLevelData, path) -> createWorldScreen.createNewWorld(layeredRegistryAccess, primaryLevelData));
    }

    private static void openCreateWorldScreen(Minecraft minecraft, @Nullable Screen screen, Function<WorldLoader.DataLoadContext, WorldGenSettings> function, WorldCreationContextMapper worldCreationContextMapper, ResourceKey<WorldPreset> resourceKey, CreateWorldCallback createWorldCallback) {
        ...
        CompletableFuture<WorldCreationContext> completableFuture = WorldLoader.load(initConfig, dataLoadContext -> new WorldLoader.DataLoadOutput<DataPackReloadCookie>(new DataPackReloadCookie((WorldGenSettings)function.apply(dataLoadContext), dataLoadContext.dataConfiguration()), dataLoadContext.datapackDimensions()), (closeableResourceManager, reloadableServerResources, layeredRegistryAccess, dataPackReloadCookie) -> {
            ...
            return worldCreationContextMapper.apply(reloadableServerResources, layeredRegistryAccess, (DataPackReloadCookie)dataPackReloadCookie);
        }, Util.backgroundExecutor(), minecraft);
        minecraft.managedBlock(completableFuture::isDone);
        minecraft.setScreen(new CreateWorldScreen(minecraft, screen, completableFuture.join(), Optional.of(resourceKey), OptionalLong.empty(), createWorldCallback));
    }

    public static CreateWorldScreen createFromExisting(Minecraft minecraft, @Nullable Screen screen, LevelSettings levelSettings, WorldCreationContext worldCreationContext, @Nullable Path path2) {
        CreateWorldScreen createWorldScreen2 = new CreateWorldScreen(minecraft, screen, worldCreationContext, WorldPresets.fromSettings(worldCreationContext.selectedDimensions()), OptionalLong.of(worldCreationContext.options().seed()), (createWorldScreen, layeredRegistryAccess, primaryLevelData, path) -> createWorldScreen.createNewWorld(layeredRegistryAccess, primaryLevelData));
        ...
        createWorldScreen2.uiState.getGameRules().assignFrom(levelSettings.gameRules(), null);
    ...
    private CreateWorldScreen(Minecraft minecraft, @Nullable Screen screen, WorldCreationContext worldCreationContext, Optional<ResourceKey<WorldPreset>> optional, OptionalLong optionalLong, CreateWorldCallback createWorldCallback) {
    ...
    public void repositionElements() {
        ...
        int n = this.tabNavigationBar.getRectangle().bottom();
        ScreenRectangle screenRectangle = new ScreenRectangle(0, n, this.width, this.height - this.layout.getFooterHeight() - n);
        this.tabManager.setTabArea(screenRectangle);
        this.layout.setHeaderHeight(n);
    ...
    public boolean keyPressed(int n, int n2, int n3) {
        if (this.tabNavigationBar.keyPressed(n)) {
            return true;
        }
    ...
    private void tryApplyNewDataPacks(PackRepository packRepository, boolean bl2, Consumer<WorldDataConfiguration> consumer) {
        ...
        WorldDataConfiguration worldDataConfiguration = new WorldDataConfiguration(new DataPackConfig((List<String>)immutableList, list = (List)packRepository.getAvailableIds().stream().filter(arg_0 -> CreateWorldScreen.lambda$tryApplyNewDataPacks$17((List)immutableList, arg_0)).collect(ImmutableList.toImmutableList())), this.uiState.getSettings().dataConfiguration().enabledFeatures());
    ...
    private void applyNewPackConfig(PackRepository packRepository, WorldDataConfiguration worldDataConfiguration, Consumer<WorldDataConfiguration> consumer) {
        ...
        ((CompletableFuture)((CompletableFuture)WorldLoader.load(initConfig, dataLoadContext -> {
            ...
            WorldCreationContext worldCreationContext = this.uiState.getSettings();
            RegistryOps registryOps = worldCreationContext.worldgenLoadContext().createSerializationContext(JsonOps.INSTANCE);
            DataResult dataResult = WorldGenSettings.encode(registryOps, worldCreationContext.options(), worldCreationContext.selectedDimensions()).setLifecycle(Lifecycle.stable());
            RegistryOps registryOps2 = dataLoadContext.datapackWorldgen().createSerializationContext(JsonOps.INSTANCE);
            WorldGenSettings worldGenSettings = (WorldGenSettings)dataResult.flatMap(jsonElement -> WorldGenSettings.CODEC.parse(registryOps2, jsonElement)).getOrThrow(string -> new IllegalStateException("Error parsing worldgen settings after loading data packs: " + string));
    ...
    private static Optional<LevelStorageSource.LevelStorageAccess> createNewWorldDirectory(Minecraft minecraft, String string, @Nullable Path path) {
        ...
        block12: {
            ...
            try {
                Path path4 = levelStorageAccess.getLevelPath(LevelResource.DATAPACK_DIR);
                FileUtil.createDirectoriesSafe(path4);
                stream.filter(path2 -> !path2.equals(path)).forEach(path3 -> CreateWorldScreen.copyBetweenDirs(path, path4, path3));
    ...
    private static /* synthetic */ boolean lambda$tryApplyNewDataPacks$17(List list, String string) {
    ...
    extends GridLayoutTab {
        ...
        GameTab() {
            ...
            CycleButton<Difficulty> cycleButton3 = rowHelper.addChild(CycleButton.builder(Difficulty::getDisplayName).withValues((Difficulty[])Difficulty.values()).create(0, 0, 210, 20, Component.translatable("options.difficulty"), (cycleButton, difficulty) -> CreateWorldScreen.this.uiState.setDifficulty((Difficulty)difficulty)), layoutSettings);
            CreateWorldScreen.this.uiState.addListener(worldCreationUiState -> {
            ...
            CycleButton<Boolean> cycleButton4 = rowHelper.addChild(CycleButton.onOffBuilder().withTooltip(bl -> Tooltip.create(ALLOW_COMMANDS_INFO)).create(0, 0, 210, 20, ALLOW_COMMANDS, (cycleButton, bl) -> CreateWorldScreen.this.uiState.setAllowCommands((boolean)bl)));
            CreateWorldScreen.this.uiState.addListener(worldCreationUiState -> {
                cycleButton4.setValue(CreateWorldScreen.this.uiState.isAllowCommands());
