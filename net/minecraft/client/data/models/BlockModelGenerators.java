...
public class BlockModelGenerators {
    ...
    static final Map<Block, BlockStateGeneratorSupplier> FULL_BLOCK_MODEL_CUSTOM_GENERATORS = Map.of(Blocks.STONE, BlockModelGenerators::createMirroredCubeGenerator, Blocks.DEEPSLATE, BlockModelGenerators::createMirroredColumnGenerator, Blocks.MUD_BRICKS, BlockModelGenerators::createNorthWestMirroredCubeGenerator);
    ...
    private static MultiVariant variants(Variant ... variantArray) {
        return new MultiVariant(WeightedList.of(Arrays.stream(variantArray).map(variant -> new Weighted<Variant>((Variant)variant, 1)).toList()));
    }

    static MultiVariant plainVariant(ResourceLocation resourceLocation) {
        return BlockModelGenerators.variant(BlockModelGenerators.plainModel(resourceLocation));
    }
    ...
    private static Condition or(ConditionBuilder ... conditionBuilderArray) {
        return new CombinedCondition(CombinedCondition.Operation.OR, Stream.of(conditionBuilderArray).map(ConditionBuilder::build).toList());
    }

    private static BlockModelDefinitionGenerator createMirroredCubeGenerator(Block block, Variant variant, TextureMapping textureMapping, BiConsumer<ResourceLocation, ModelInstance> biConsumer) {
    ...
    private static BlockModelDefinitionGenerator createNorthWestMirroredCubeGenerator(Block block, Variant variant, TextureMapping textureMapping, BiConsumer<ResourceLocation, ModelInstance> biConsumer) {
    ...
    private static BlockModelDefinitionGenerator createMirroredColumnGenerator(Block block, Variant variant, TextureMapping textureMapping, BiConsumer<ResourceLocation, ModelInstance> biConsumer) {
    ...
    public BlockModelGenerators(Consumer<BlockModelDefinitionGenerator> consumer, ItemModelOutput itemModelOutput, BiConsumer<ResourceLocation, ModelInstance> biConsumer) {
    ...
    ResourceLocation createFlatItemModelWithBlockTextureAndOverlay(Item item, Block block, String string) {
        ...
        return ModelTemplates.TWO_LAYERED_ITEM.create(ModelLocationUtils.getModelLocation(item), TextureMapping.layered(resourceLocation, resourceLocation2), this.modelOutput);
    }
    ...
    private void createBrushableBlock(Block block) {
        this.blockStateOutput.accept(MultiVariantGenerator.dispatch(block).with(PropertyDispatch.initial(BlockStateProperties.DUSTED).generate(n -> {
            String string = "_" + n;
            ResourceLocation resourceLocation = TextureMapping.getBlockTexture(block, string);
            ResourceLocation resourceLocation2 = ModelTemplates.CUBE_ALL.createWithSuffix(block, string, new TextureMapping().put(TextureSlot.ALL, resourceLocation), this.modelOutput);
            return BlockModelGenerators.plainVariant(resourceLocation2);
        })));
    ...
    static BlockModelDefinitionGenerator createPillarBlockUVLocked(Block block, TextureMapping textureMapping, BiConsumer<ResourceLocation, ModelInstance> biConsumer) {
    ...
    public void createTintedLeaves(Block block, TexturedModel.Provider provider, int n) {
        ResourceLocation resourceLocation = provider.create(block, this.modelOutput);
        this.blockStateOutput.accept(BlockModelGenerators.createSimpleBlock(block, BlockModelGenerators.plainVariant(resourceLocation)));
        this.registerSimpleTintedItemModel(block, resourceLocation, ItemModelUtils.constantTint(n));
    }
    ...
    void createOrientableTrapdoor(Block block) {
        ...
        this.blockStateOutput.accept(BlockModelGenerators.createOrientableTrapdoor(block, multiVariant, BlockModelGenerators.plainVariant(resourceLocation), multiVariant2));
    ...
    void createTrapdoor(Block block) {
        ...
        this.blockStateOutput.accept(BlockModelGenerators.createTrapdoor(block, multiVariant, BlockModelGenerators.plainVariant(resourceLocation), multiVariant2));
        this.registerSimpleItemModel(block, resourceLocation);
    }
    ...
    private void createCrossBlock(Block block, PlantType plantType, Property<Integer> property, int ... nArray) {
        if (property.getPossibleValues().size() != nArray.length) {
            throw new IllegalArgumentException("missing values for property: " + String.valueOf(property));
        }
        this.registerSimpleFlatItemModel(block.asItem());
        this.blockStateOutput.accept(MultiVariantGenerator.dispatch(block).with(PropertyDispatch.initial(property).generate(n -> {
    ...
    private void createStems(Block block, Block block2) {
        ...
        this.blockStateOutput.accept(MultiVariantGenerator.dispatch(block).with(PropertyDispatch.initial(BlockStateProperties.AGE_7).generate(n -> BlockModelGenerators.plainVariant(ModelTemplates.STEMS[n].create(block, textureMapping, this.modelOutput)))));
    }
    ...
    private void createPitcherCrop() {
        ...
        this.registerSimpleFlatItemModel(block.asItem());
        this.blockStateOutput.accept(MultiVariantGenerator.dispatch(block).with(PropertyDispatch.initial(PitcherCropBlock.AGE, BlockStateProperties.DOUBLE_BLOCK_HALF).generate((n, doubleBlockHalf) -> switch (doubleBlockHalf) {
            default -> throw new MatchException(null, null);
            case DoubleBlockHalf.UPPER -> BlockModelGenerators.plainVariant(ModelLocationUtils.getModelLocation(block, "_top_stage_" + n));
            case DoubleBlockHalf.LOWER -> BlockModelGenerators.plainVariant(ModelLocationUtils.getModelLocation(block, "_bottom_stage_" + n));
        })));
    }
    ...
    private void createActiveRail(Block block) {
        ...
        this.registerSimpleFlatItemModel(block);
        this.blockStateOutput.accept(MultiVariantGenerator.dispatch(block).with(PropertyDispatch.initial(BlockStateProperties.POWERED, BlockStateProperties.RAIL_SHAPE_STRAIGHT).generate((bl, railShape) -> switch (railShape) {
            ...
            case RailShape.EAST_WEST -> (bl != false ? multiVariant4 : multiVariant).with(Y_ROT_90);
            case RailShape.ASCENDING_EAST -> (bl != false ? multiVariant5 : multiVariant2).with(Y_ROT_90);
            case RailShape.ASCENDING_WEST -> (bl != false ? multiVariant6 : multiVariant3).with(Y_ROT_90);
    ...
    private void createAirLikeBlock(Block block, ResourceLocation resourceLocation) {
        MultiVariant multiVariant = BlockModelGenerators.plainVariant(ModelTemplates.PARTICLE_ONLY.create(block, TextureMapping.particle(resourceLocation), this.modelOutput));
        this.blockStateOutput.accept(BlockModelGenerators.createSimpleBlock(block, multiVariant));
    }
    ...
    private void createColoredBlockWithRandomRotations(TexturedModel.Provider provider, Block ... blockArray) {
    ...
    private void createColoredBlockWithStateRotations(TexturedModel.Provider provider, Block ... blockArray) {
        for (Block block : blockArray) {
            MultiVariant multiVariant = BlockModelGenerators.plainVariant(provider.create(block, this.modelOutput));
    ...
    private void createCommandBlock(Block block) {
        ...
        MultiVariant multiVariant2 = BlockModelGenerators.plainVariant(this.createSuffixedVariant(block, "_conditional", ModelTemplates.COMMAND_BLOCK, resourceLocation -> textureMapping.copyAndUpdate(TextureSlot.SIDE, (ResourceLocation)resourceLocation)));
    ...
    private static MultiVariant createBambooModels(int n2) {
        String string = "_age" + n2;
        return new MultiVariant(WeightedList.of(IntStream.range(1, 5).mapToObj(n -> new Weighted<Variant>(BlockModelGenerators.plainModel(ModelLocationUtils.getModelLocation(Blocks.BAMBOO, n + string)), 1)).collect(Collectors.toList())));
    }
    ...
    private void createBarrel() {
        ...
        MultiVariant multiVariant2 = BlockModelGenerators.plainVariant(TexturedModel.CUBE_TOP_BOTTOM.get(Blocks.BARREL).updateTextures(textureMapping -> textureMapping.put(TextureSlot.TOP, resourceLocation)).createWithSuffix(Blocks.BARREL, "_open", this.modelOutput));
    ...
    private static <T extends Comparable<T>> PropertyDispatch<MultiVariant> createEmptyOrFullDispatch(Property<T> property, T t, MultiVariant multiVariant, MultiVariant multiVariant2) {
        return PropertyDispatch.initial(property).generate(comparable2 -> {
            boolean bl = comparable2.compareTo(t) >= 0;
    ...
    private void createBeeNest(Block block, Function<Block, TextureMapping> function) {
        ...
        this.itemModelOutput.accept(block.asItem(), ItemModelUtils.selectBlockItemProperty(BeehiveBlock.HONEY_LEVEL, ItemModelUtils.plainModel(resourceLocation), Map.of(5, ItemModelUtils.plainModel(resourceLocation2))));
        this.blockStateOutput.accept(MultiVariantGenerator.dispatch(block).with(BlockModelGenerators.createEmptyOrFullDispatch(BeehiveBlock.HONEY_LEVEL, 5, BlockModelGenerators.plainVariant(resourceLocation2), BlockModelGenerators.plainVariant(resourceLocation))).with(ROTATION_HORIZONTAL_FACING));
    }

    private void createCropBlock(Block block, Property<Integer> property, int ... nArray) {
        this.registerSimpleFlatItemModel(block.asItem());
        if (property.getPossibleValues().size() != nArray.length) {
            throw new IllegalArgumentException();
        }
        Int2ObjectOpenHashMap int2ObjectOpenHashMap = new Int2ObjectOpenHashMap();
        this.blockStateOutput.accept(MultiVariantGenerator.dispatch(block).with(PropertyDispatch.initial(property).generate(arg_0 -> this.lambda$createCropBlock$26(nArray, (Int2ObjectMap)int2ObjectOpenHashMap, block, arg_0))));
    }
    ...
    private void createFurnace(Block block, TexturedModel.Provider provider) {
        ...
        MultiVariant multiVariant2 = BlockModelGenerators.plainVariant(provider.get(block).updateTextures(textureMapping -> textureMapping.put(TextureSlot.FRONT, resourceLocation)).createWithSuffix(block, "_on", this.modelOutput));
    ...
    private void createCampfires(Block ... blockArray) {
    ...
    private void createChorusFlower() {
        ...
        MultiVariant multiVariant2 = BlockModelGenerators.plainVariant(this.createSuffixedVariant(Blocks.CHORUS_FLOWER, "_dead", ModelTemplates.CHORUS_FLOWER, resourceLocation -> textureMapping.copyAndUpdate(TextureSlot.TEXTURE, (ResourceLocation)resourceLocation)));
    ...
    private static BlockModelDefinitionGenerator createCopperBulb(Block block, MultiVariant multiVariant, MultiVariant multiVariant2, MultiVariant multiVariant3, MultiVariant multiVariant4) {
        return MultiVariantGenerator.dispatch(block).with(PropertyDispatch.initial(BlockStateProperties.LIT, BlockStateProperties.POWERED).generate((bl, bl2) -> {
            if (bl.booleanValue()) {
                return bl2 != false ? multiVariant4 : multiVariant2;
            }
            return bl2 != false ? multiVariant3 : multiVariant;
        }));
    }
    ...
    private void createLightningRod() {
        ...
        MultiVariant multiVariant = BlockModelGenerators.plainVariant(ModelLocationUtils.getModelLocation(block, "_on"));
        MultiVariant multiVariant2 = BlockModelGenerators.plainVariant(ModelLocationUtils.getModelLocation(block));
    ...
    private void createMangrovePropagule() {
        ...
        MultiVariant multiVariant = BlockModelGenerators.plainVariant(ModelLocationUtils.getModelLocation(block));
        this.blockStateOutput.accept(MultiVariantGenerator.dispatch(Blocks.MANGROVE_PROPAGULE).with(PropertyDispatch.initial(MangrovePropaguleBlock.HANGING, MangrovePropaguleBlock.AGE).generate((bl, n) -> bl != false ? BlockModelGenerators.plainVariant(ModelLocationUtils.getModelLocation(block, "_hanging_" + n)) : multiVariant)));
    }
    ...
    private void createGrassBlocks() {
        ...
        this.createGrassLikeBlock(Blocks.GRASS_BLOCK, BlockModelGenerators.createRotatedVariants(BlockModelGenerators.plainModel(resourceLocation2)), multiVariant);
        this.registerSimpleTintedItemModel(Blocks.GRASS_BLOCK, resourceLocation2, new GrassColorSource());
        MultiVariant multiVariant2 = BlockModelGenerators.createRotatedVariants(BlockModelGenerators.plainModel(TexturedModel.CUBE_TOP_BOTTOM.get(Blocks.MYCELIUM).updateTextures(textureMapping -> textureMapping.put(TextureSlot.BOTTOM, resourceLocation)).create(Blocks.MYCELIUM, this.modelOutput)));
        this.createGrassLikeBlock(Blocks.MYCELIUM, multiVariant2, multiVariant);
        MultiVariant multiVariant3 = BlockModelGenerators.createRotatedVariants(BlockModelGenerators.plainModel(TexturedModel.CUBE_TOP_BOTTOM.get(Blocks.PODZOL).updateTextures(textureMapping -> textureMapping.put(TextureSlot.BOTTOM, resourceLocation)).create(Blocks.PODZOL, this.modelOutput)));
    ...
    private void createPistons() {
        ...
        ResourceLocation resourceLocation3 = ModelTemplates.CUBE_BOTTOM_TOP.createWithSuffix(Blocks.PISTON, "_inventory", textureMapping.copyAndUpdate(TextureSlot.TOP, resourceLocation2), this.modelOutput);
    ...
    private void createTrialSpawner() {
        ...
        this.registerSimpleItemModel(block, resourceLocation);
        this.blockStateOutput.accept(MultiVariantGenerator.dispatch(block).with(PropertyDispatch.initial(BlockStateProperties.TRIAL_SPAWNER_STATE, BlockStateProperties.OMINOUS).generate((trialSpawnerState, bl) -> switch (trialSpawnerState) {
            default -> throw new MatchException(null, null);
            ...
            case TrialSpawnerState.EJECTING_REWARD -> bl != false ? multiVariant6 : multiVariant3;
        })));
    }
    ...
    private void createVault() {
        ...
        this.registerSimpleItemModel(block, resourceLocation);
        this.blockStateOutput.accept(MultiVariantGenerator.dispatch(block).with(PropertyDispatch.initial(VaultBlock.STATE, VaultBlock.OMINOUS).generate((vaultState, bl) -> switch (vaultState) {
            default -> throw new MatchException(null, null);
            ...
            case VaultState.EJECTING -> bl != false ? multiVariant8 : multiVariant4;
    ...
    private void createRepeater() {
        ...
        this.blockStateOutput.accept(MultiVariantGenerator.dispatch(Blocks.REPEATER).with(PropertyDispatch.initial(BlockStateProperties.DELAY, BlockStateProperties.LOCKED, BlockStateProperties.POWERED).generate((Function3<Integer, Boolean, Boolean, MultiVariant>)((Function3)(n, bl, bl2) -> {
    ...
    private void createSnowBlocks() {
        ...
        this.blockStateOutput.accept(MultiVariantGenerator.dispatch(Blocks.SNOW).with(PropertyDispatch.initial(BlockStateProperties.LAYERS).generate(n -> n < 8 ? BlockModelGenerators.plainVariant(ModelLocationUtils.getModelLocation(Blocks.SNOW, "_height" + n * 2)) : multiVariant)));
    ...
    private void createTestBlock() {
        HashMap<TestBlockMode, ResourceLocation> hashMap = new HashMap<TestBlockMode, ResourceLocation>();
        for (TestBlockMode testBlockMode2 : TestBlockMode.values()) {
            hashMap.put(testBlockMode2, this.createSuffixedVariant(Blocks.TEST_BLOCK, "_" + testBlockMode2.getSerializedName(), ModelTemplates.CUBE_ALL, TextureMapping::cube));
        }
        this.blockStateOutput.accept(MultiVariantGenerator.dispatch(Blocks.TEST_BLOCK).with(PropertyDispatch.initial(BlockStateProperties.TEST_BLOCK_MODE).generate(testBlockMode -> BlockModelGenerators.plainVariant((ResourceLocation)hashMap.get(testBlockMode)))));
        this.itemModelOutput.accept(Items.TEST_BLOCK, ItemModelUtils.selectBlockItemProperty(TestBlock.MODE, ItemModelUtils.plainModel((ResourceLocation)hashMap.get(TestBlockMode.START)), Map.of(TestBlockMode.FAIL, ItemModelUtils.plainModel((ResourceLocation)hashMap.get(TestBlockMode.FAIL)), TestBlockMode.LOG, ItemModelUtils.plainModel((ResourceLocation)hashMap.get(TestBlockMode.LOG)), TestBlockMode.ACCEPT, ItemModelUtils.plainModel((ResourceLocation)hashMap.get(TestBlockMode.ACCEPT)))));
    }

    private void createSweetBerryBush() {
        this.registerSimpleFlatItemModel(Items.SWEET_BERRIES);
        this.blockStateOutput.accept(MultiVariantGenerator.dispatch(Blocks.SWEET_BERRY_BUSH).with(PropertyDispatch.initial(BlockStateProperties.AGE_3).generate(n -> BlockModelGenerators.plainVariant(this.createSuffixedVariant(Blocks.SWEET_BERRY_BUSH, "_stage" + n, ModelTemplates.CROSS, TextureMapping::cross)))));
    }
    ...
    private void createTripwireHook() {
        ...
        this.blockStateOutput.accept(MultiVariantGenerator.dispatch(Blocks.TRIPWIRE_HOOK).with(PropertyDispatch.initial(BlockStateProperties.ATTACHED, BlockStateProperties.POWERED).generate((bl, bl2) -> BlockModelGenerators.plainVariant(ModelLocationUtils.getModelLocation(Blocks.TRIPWIRE_HOOK, (bl != false ? "_attached" : "") + (bl2 != false ? "_on" : ""))))).with(ROTATION_HORIZONTAL_FACING));
    }

    private Variant createTurtleEggModel(int n, String string, TextureMapping textureMapping) {
        return switch (n) {
    ...
    private Variant createTurtleEggModel(int n, int n2) {
        return switch (n2) {
            case 0 -> this.createTurtleEggModel(n, "", TextureMapping.cube(TextureMapping.getBlockTexture(Blocks.TURTLE_EGG)));
            case 1 -> this.createTurtleEggModel(n, "slightly_cracked_", TextureMapping.cube(TextureMapping.getBlockTexture(Blocks.TURTLE_EGG, "_slightly_cracked")));
            case 2 -> this.createTurtleEggModel(n, "very_cracked_", TextureMapping.cube(TextureMapping.getBlockTexture(Blocks.TURTLE_EGG, "_very_cracked")));
            default -> throw new UnsupportedOperationException();
        };
    }
    ...
    private void createTurtleEgg() {
        ...
        this.blockStateOutput.accept(MultiVariantGenerator.dispatch(Blocks.TURTLE_EGG).with(PropertyDispatch.initial(BlockStateProperties.EGGS, BlockStateProperties.HATCH).generate((n, n2) -> BlockModelGenerators.createRotatedVariants(this.createTurtleEggModel((int)n, (int)n2)))));
    }
    ...
    private void createDriedGhastBlock() {
        ...
        Function<Integer, ResourceLocation> function = n -> {
        ...
        this.blockStateOutput.accept(MultiVariantGenerator.dispatch(Blocks.DRIED_GHAST).with(PropertyDispatch.initial(DriedGhastBlock.HYDRATION_LEVEL).generate(n -> BlockModelGenerators.plainVariant((ResourceLocation)function.apply((Integer)n)))).with(ROTATION_HORIZONTAL_FACING));
    }

    private void createSnifferEgg() {
        this.registerSimpleFlatItemModel(Items.SNIFFER_EGG);
        this.blockStateOutput.accept(MultiVariantGenerator.dispatch(Blocks.SNIFFER_EGG).with(PropertyDispatch.initial(SnifferEggBlock.HATCH).generate(n -> {
            String string = switch (n) {
    ...
    private void createHangingMoss(Block block) {
        this.registerSimpleFlatItemModel(block);
        this.blockStateOutput.accept(MultiVariantGenerator.dispatch(block).with(PropertyDispatch.initial(HangingMossBlock.TIP).generate(bl -> {
    ...
    private void createSculkCatalyst() {
        ...
        MultiVariant multiVariant = BlockModelGenerators.plainVariant(resourceLocation2);
        ...
        this.blockStateOutput.accept(MultiVariantGenerator.dispatch(Blocks.SCULK_CATALYST).with(PropertyDispatch.initial(BlockStateProperties.BLOOM).generate(bl -> bl != false ? multiVariant2 : multiVariant)));
    ...
    private void addSlotStateAndRotationVariants(MultiPartGenerator multiPartGenerator, Condition condition, VariantMutator variantMutator) {
        List.of(Pair.of((Object)BlockStateProperties.CHISELED_BOOKSHELF_SLOT_0_OCCUPIED, (Object)ModelTemplates.CHISELED_BOOKSHELF_SLOT_TOP_LEFT), Pair.of((Object)BlockStateProperties.CHISELED_BOOKSHELF_SLOT_1_OCCUPIED, (Object)ModelTemplates.CHISELED_BOOKSHELF_SLOT_TOP_MID), Pair.of((Object)BlockStateProperties.CHISELED_BOOKSHELF_SLOT_2_OCCUPIED, (Object)ModelTemplates.CHISELED_BOOKSHELF_SLOT_TOP_RIGHT), Pair.of((Object)BlockStateProperties.CHISELED_BOOKSHELF_SLOT_3_OCCUPIED, (Object)ModelTemplates.CHISELED_BOOKSHELF_SLOT_BOTTOM_LEFT), Pair.of((Object)BlockStateProperties.CHISELED_BOOKSHELF_SLOT_4_OCCUPIED, (Object)ModelTemplates.CHISELED_BOOKSHELF_SLOT_BOTTOM_MID), Pair.of((Object)BlockStateProperties.CHISELED_BOOKSHELF_SLOT_5_OCCUPIED, (Object)ModelTemplates.CHISELED_BOOKSHELF_SLOT_BOTTOM_RIGHT)).forEach(pair -> {
    ...
    private void createShulkerBox(Block block, @Nullable DyeColor dyeColor) {
        ...
        ItemModel.Unbaked unbaked = dyeColor != null ? ItemModelUtils.specialModel(resourceLocation, new ShulkerBoxSpecialRenderer.Unbaked(dyeColor)) : ItemModelUtils.specialModel(resourceLocation, new ShulkerBoxSpecialRenderer.Unbaked());
    ...
    private void createInfestedStone() {
        ...
        Variant variant = BlockModelGenerators.plainModel(resourceLocation);
    ...
    private void createRespawnAnchor() {
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            resourceLocationArray[i] = ModelTemplates.CUBE_BOTTOM_TOP.createWithSuffix(Blocks.RESPAWN_ANCHOR, "_" + i, textureMapping, this.modelOutput);
        }
        this.blockStateOutput.accept(MultiVariantGenerator.dispatch(Blocks.RESPAWN_ANCHOR).with(PropertyDispatch.initial(BlockStateProperties.RESPAWN_ANCHOR_CHARGES).generate(n -> BlockModelGenerators.plainVariant(resourceLocationArray[n]))));
        this.registerSimpleItemModel(Blocks.RESPAWN_ANCHOR, resourceLocationArray[0]);
    }
    ...
    private void createBed(Block block, Block block2, DyeColor dyeColor) {
        ...
        this.blockStateOutput.accept(BlockModelGenerators.createSimpleBlock(block, multiVariant));
        Item item = block.asItem();
        ResourceLocation resourceLocation = ModelTemplates.BED_INVENTORY.create(ModelLocationUtils.getModelLocation(item), TextureMapping.particle(block2), this.modelOutput);
    ...
    private void createLightBlock() {
        ...
        HashMap<Integer, ItemModel.Unbaked> hashMap = new HashMap<Integer, ItemModel.Unbaked>(16);
        ...
        for (int i = 0; i <= 15; ++i) {
            ...
            c1.select(i, BlockModelGenerators.plainVariant(ModelTemplates.PARTICLE_ONLY.createWithSuffix(Blocks.LIGHT, string, TextureMapping.particle(resourceLocation), this.modelOutput)));
            ItemModel.Unbaked unbaked2 = ItemModelUtils.plainModel(ModelTemplates.FLAT_ITEM.create(ModelLocationUtils.getModelLocation(Items.LIGHT, string), TextureMapping.layer0(resourceLocation), this.modelOutput));
            hashMap.put(i, unbaked2);
        }
        this.itemModelOutput.accept(Items.LIGHT, ItemModelUtils.selectBlockItemProperty(LightBlock.LEVEL, unbaked, hashMap));
    ...
    private /* synthetic */ MultiVariant lambda$createCropBlock$26(int[] nArray, Int2ObjectMap int2ObjectMap, Block block, Integer n2) {
        int n3 = nArray[n2];
        return BlockModelGenerators.plainVariant((ResourceLocation)int2ObjectMap.computeIfAbsent(n3, n -> this.createSuffixedVariant(block, "_stage" + n, ModelTemplates.CROP, TextureMapping::crop)));
    }
    ...
    class BlockFamilyProvider {
        ...
        public BlockFamilyProvider slab(Block block) {
            ...
            BlockModelGenerators.this.blockStateOutput.accept(BlockModelGenerators.createSlab(block, BlockModelGenerators.plainVariant(resourceLocation), multiVariant, BlockModelGenerators.variant(this.fullBlock)));
            BlockModelGenerators.this.registerSimpleItemModel(block, resourceLocation);
            return this;
        }
        ...
        public BlockFamilyProvider stairs(Block block) {
            ...
            BlockModelGenerators.this.blockStateOutput.accept(BlockModelGenerators.createStairs(block, multiVariant, BlockModelGenerators.plainVariant(resourceLocation), multiVariant2));
            BlockModelGenerators.this.registerSimpleItemModel(block, resourceLocation);
            return this;
        }
    ...
    class WoodProvider {
        ...
        public WoodProvider wood(Block block) {
            ...
            BlockModelGenerators.this.blockStateOutput.accept(BlockModelGenerators.createAxisAlignedPillarBlock(block, BlockModelGenerators.plainVariant(resourceLocation)));
            BlockModelGenerators.this.registerSimpleItemModel(block, resourceLocation);
            return this;
        }
        ...
        public WoodProvider logWithHorizontal(Block block) {
            ...
            BlockModelGenerators.this.blockStateOutput.accept(BlockModelGenerators.createRotatedPillarWithHorizontalVariant(block, BlockModelGenerators.plainVariant(resourceLocation), multiVariant));
            BlockModelGenerators.this.registerSimpleItemModel(block, resourceLocation);
            return this;
        }
