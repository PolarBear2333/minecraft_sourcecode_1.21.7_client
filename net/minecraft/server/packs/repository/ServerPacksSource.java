...
extends BuiltInPackSource {
    ...
    public ServerPacksSource(DirectoryValidator directoryValidator) {
        super(PackType.SERVER_DATA, ServerPacksSource.createVanillaPackSource(), PACKS_DIR, directoryValidator);
    }
    ...
    protected Pack createVanillaPack(PackResources packResources) {
        return Pack.readMetaAndCreate(VANILLA_PACK_INFO, ServerPacksSource.fixedResources(packResources), PackType.SERVER_DATA, VANILLA_SELECTION_CONFIG);
    }

    @Override
    @Nullable
    protected Pack createBuiltinPack(String string, Pack.ResourcesSupplier resourcesSupplier, Component component) {
        return Pack.readMetaAndCreate(ServerPacksSource.createBuiltInPackLocation(string, component), resourcesSupplier, PackType.SERVER_DATA, FEATURE_SELECTION_CONFIG);
    }

    public static PackRepository createPackRepository(Path path, DirectoryValidator directoryValidator) {
        return new PackRepository(new ServerPacksSource(directoryValidator), new FolderRepositorySource(path, PackType.SERVER_DATA, PackSource.WORLD, directoryValidator));
    }
    ...
    public static PackRepository createPackRepository(LevelStorageSource.LevelStorageAccess levelStorageAccess) {
        return ServerPacksSource.createPackRepository(levelStorageAccess.getLevelPath(LevelResource.DATAPACK_DIR), levelStorageAccess.parent().getWorldDirValidator());
    }
}
