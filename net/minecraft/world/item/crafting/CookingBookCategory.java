...
public enum CookingBookCategory implements StringRepresentable
{
    ...
    private CookingBookCategory(int n2, String string2) {
        this.id = n2;
        this.name = string2;
    }
    ...
    static {
        BY_ID = ByIdMap.continuous(cookingBookCategory -> cookingBookCategory.id, CookingBookCategory.values(), ByIdMap.OutOfBoundsStrategy.ZERO);
        CODEC = StringRepresentable.fromEnum(CookingBookCategory::values);
        STREAM_CODEC = ByteBufCodecs.idMapper(BY_ID, cookingBookCategory -> cookingBookCategory.id);
    }
}
