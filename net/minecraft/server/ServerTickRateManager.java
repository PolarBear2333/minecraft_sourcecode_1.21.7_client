...
extends TickRateManager {
    ...
    public boolean stepGameIfPaused(int n) {
        if (!this.isFrozen()) {
            return false;
        }
        this.frozenTicksToRun = n;
    ...
    public boolean requestGameToSprint(int n) {
        boolean bl = this.remainingSprintTicks > 0L;
        this.sprintTimeSpend = 0L;
        this.scheduledCurrentSprintTicks = n;
        this.remainingSprintTicks = n;
    ...
    private void finishTickSprint() {
        ...
        int n = (int)((double)(TimeUtil.MILLISECONDS_PER_SECOND * l) / d);
        String string = String.format("%.2f", l == 0L ? (double)this.millisecondsPerTick() : d / (double)l);
        this.scheduledCurrentSprintTicks = 0L;
        this.sprintTimeSpend = 0L;
        this.server.createCommandSourceStack().sendSuccess(() -> Component.translatable("commands.tick.sprint.report", n, string), true);
