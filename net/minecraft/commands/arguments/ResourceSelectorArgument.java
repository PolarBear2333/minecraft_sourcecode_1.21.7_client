...
implements ArgumentType<Collection<Holder.Reference<T>>> {
    ...
    public Collection<Holder.Reference<T>> parse(StringReader stringReader) throws CommandSyntaxException {
        ...
        List<Holder.Reference<T>> list = this.registryLookup.listElements().filter(reference -> ResourceSelectorArgument.matches(string, reference.key().location())).toList();
        if (list.isEmpty()) {
            throw ERROR_NO_MATCHES.createWithContext((ImmutableStringReader)stringReader, (Object)string, (Object)this.registryKey.location());
        }
        return list;
    }
    ...
    public static <T> Collection<Holder.Reference<T>> parse(StringReader stringReader, HolderLookup<T> holderLookup) {
        String string = ResourceSelectorArgument.ensureNamespaced(ResourceSelectorArgument.readPattern(stringReader));
        return holderLookup.listElements().filter(reference -> ResourceSelectorArgument.matches(string, reference.key().location())).toList();
    }
    ...
    private static boolean matches(String string, ResourceLocation resourceLocation) {
        return FilenameUtils.wildcardMatch((String)resourceLocation.toString(), (String)string);
    }
