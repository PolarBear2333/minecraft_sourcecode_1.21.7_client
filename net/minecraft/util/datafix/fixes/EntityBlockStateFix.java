...
extends DataFix {
    private static final Map<String, Integer> MAP = (Map)DataFixUtils.make((Object)Maps.newHashMap(), hashMap -> {
    ...
    public TypeRewriteRule makeRule() {
        ...
        Type type = DSL.and((Type)DSL.optional((Type)DSL.field((String)"inTile", (Type)DSL.named((String)References.BLOCK_NAME.typeName(), (Type)DSL.or((Type)DSL.intType(), NamespacedSchema.namespacedString())))), (Type)DSL.remainderType());
        Function<Typed, Typed> function3 = typed -> typed.update(type.finder(), DSL.remainderType(), Pair::getSecond);
        return this.fixTypeEverywhereTyped("EntityBlockStateFix", schema.getType(References.ENTITY), schema2.getType(References.ENTITY), typed2 -> {
    ...
    private Typed<?> updateFallingBlock(Typed<?> typed) {
        Type type = DSL.optional((Type)DSL.field((String)"Block", (Type)DSL.named((String)References.BLOCK_NAME.typeName(), (Type)DSL.or((Type)DSL.intType(), NamespacedSchema.namespacedString()))));
        Type type2 = DSL.optional((Type)DSL.field((String)"BlockState", (Type)DSL.named((String)References.BLOCK_STATE.typeName(), (Type)DSL.remainderType())));
        Dynamic dynamic = (Dynamic)typed.get(DSL.remainderFinder());
        return typed.update(type.finder(), type2, either -> {
            int n = (Integer)either.map(pair -> (Integer)((Either)pair.getSecond()).map(n -> n, EntityBlockStateFix::getBlockId), unit -> {
            ...
            int n2 = dynamic.get("Data").asInt(0) & 0xF;
            return Either.left((Object)Pair.of((Object)References.BLOCK_STATE.typeName(), BlockStateData.getTag(n << 4 | n2)));
    ...
    private Typed<?> updateBlockToBlockState(Typed<?> typed, String string, String string2, String string3) {
        Tag.TagType tagType = DSL.field((String)string, (Type)DSL.named((String)References.BLOCK_NAME.typeName(), (Type)DSL.or((Type)DSL.intType(), NamespacedSchema.namespacedString())));
        Tag.TagType tagType2 = DSL.field((String)string3, (Type)DSL.named((String)References.BLOCK_STATE.typeName(), (Type)DSL.remainderType()));
        Dynamic dynamic = (Dynamic)typed.getOrCreate(DSL.remainderFinder());
        return typed.update(tagType.finder(), (Type)tagType2, pair -> {
            int n2 = (Integer)((Either)pair.getSecond()).map(n -> n, EntityBlockStateFix::getBlockId);
            int n3 = dynamic.get(string2).asInt(0) & 0xF;
            return Pair.of((Object)References.BLOCK_STATE.typeName(), BlockStateData.getTag(n2 << 4 | n3));
