...
public class Noises {
    ...
    public static NormalNoise instantiate(HolderGetter<NormalNoise.NoiseParameters> holderGetter, PositionalRandomFactory positionalRandomFactory, ResourceKey<NormalNoise.NoiseParameters> resourceKey) {
        Holder.Reference<NormalNoise.NoiseParameters> reference = holderGetter.getOrThrow(resourceKey);
        return NormalNoise.create(positionalRandomFactory.fromHashOf(reference.unwrapKey().orElseThrow().location()), (NormalNoise.NoiseParameters)reference.value());
    }
}
