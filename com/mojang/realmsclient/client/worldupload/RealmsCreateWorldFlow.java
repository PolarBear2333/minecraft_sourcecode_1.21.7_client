...
public class RealmsCreateWorldFlow {
    ...
    public static void createWorld(Minecraft minecraft, Screen screen, Screen screen2, int n, RealmsServer realmsServer, @Nullable RealmCreationTask realmCreationTask) {
        CreateWorldScreen.openFresh(minecraft, screen, (createWorldScreen, layeredRegistryAccess, primaryLevelData, path) -> {
            ...
            RealmsWorldOptions realmsWorldOptions = RealmsWorldOptions.createFromSettings(primaryLevelData.getLevelSettings(), primaryLevelData.getLevelSettings().allowCommands(), SharedConstants.getCurrentVersion().name());
            RealmsSlot realmsSlot = new RealmsSlot(n, realmsWorldOptions, List.of(RealmsSetting.hardcoreSetting(primaryLevelData.getLevelSettings().hardcore())));
