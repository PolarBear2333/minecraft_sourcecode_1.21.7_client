...
public class EventLogDirectory {
    ...
    private File parseFile(Path path) {
        ...
        FileId fileId = FileId.parse(string.substring(0, n));
        if (fileId != null) {
    ...
    public RawFile createNewFile(LocalDate localDate) throws IOException {
        ...
        while (set.contains(fileId = new FileId(localDate, n++))) {
        }
    ...
    implements Iterable<File> {
        ...
        public FileList prune(LocalDate localDate, int n) {
            this.files.removeIf(file -> {
                FileId fileId = file.id();
                LocalDate localDate2 = fileId.date().plusDays(n);
