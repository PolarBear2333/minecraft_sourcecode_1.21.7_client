...
public class QuickPlay {
    ...
    private static void joinSingleplayerWorld(Minecraft minecraft, @Nullable String string) {
        if (StringUtil.isBlank(string) || !minecraft.getLevelSource().levelExists(string)) {
            ...
            minecraft.setScreen(new DisconnectedScreen((Screen)selectWorldScreen, ERROR_TITLE, INVALID_IDENTIFIER, TO_WORLD_LIST));
            return;
        }
    ...
    private static void joinRealmsWorld(Minecraft minecraft, RealmsClient realmsClient, String string) {
        ...
        try {
            ...
            minecraft.setScreen(new DisconnectedScreen((Screen)realmsMainScreen, ERROR_TITLE, INVALID_IDENTIFIER, TO_REALMS_LIST));
            return;
        }
        catch (RealmsServiceException realmsServiceException) {
            TitleScreen titleScreen = new TitleScreen();
            minecraft.setScreen(new DisconnectedScreen((Screen)titleScreen, ERROR_TITLE, REALM_CONNECT, TO_TITLE));
            return;
        }
        ...
        if (realmsServer2 == null) {
            RealmsMainScreen realmsMainScreen = new RealmsMainScreen(new TitleScreen());
            minecraft.setScreen(new DisconnectedScreen((Screen)realmsMainScreen, ERROR_TITLE, REALM_PERMISSION, TO_REALMS_LIST));
            return;
        }
        TitleScreen titleScreen = new TitleScreen();
