...
public class WorldOpenFlows {
    ...
    public void createFreshLevel(String string, LevelSettings levelSettings, WorldOptions worldOptions, Function<HolderLookup.Provider, WorldDimensions> function, Screen screen) {
        ...
        try {
            ...
            WorldStem worldStem = this.loadWorldDataBlocking(packConfig, dataLoadContext -> {
                WorldDimensions.Complete complete = ((WorldDimensions)function.apply(dataLoadContext.datapackWorldgen())).bake((Registry<LevelStem>)dataLoadContext.datapackDimensions().lookupOrThrow(Registries.LEVEL_STEM));
    ...
    public Pair<LevelSettings, WorldCreationContext> recreateWorldData(LevelStorageSource.LevelStorageAccess levelStorageAccess) throws Exception {
        ...
        return this.loadWorldDataBlocking(packConfig, dataLoadContext -> {
        ...
        }, (closeableResourceManager, reloadableServerResources, layeredRegistryAccess, data) -> {
            ...
            return Pair.of((Object)data.levelSettings, (Object)new WorldCreationContext(data.options, new WorldDimensions(data.existingDimensions), layeredRegistryAccess, reloadableServerResources, data.levelSettings.getDataConfiguration(), initialWorldCreationOptions));
        });
    }
    ...
    private void askForBackup(LevelStorageSource.LevelStorageAccess levelStorageAccess, boolean bl3, Runnable runnable, Runnable runnable2) {
        ...
        if (bl3) {
            mutableComponent2 = Component.translatable("selectWorld.backupQuestion.customized");
            mutableComponent = Component.translatable("selectWorld.backupWarning.customized");
        } else {
            mutableComponent2 = Component.translatable("selectWorld.backupQuestion.experimental");
            mutableComponent = Component.translatable("selectWorld.backupWarning.experimental");
        }
        this.minecraft.setScreen(new BackupConfirmScreen(runnable2, (bl, bl2) -> {
            ...
            runnable.run();
        }, mutableComponent2, mutableComponent, false));
    }
    ...
    private void openWorldCheckVersionCompatibility(LevelStorageSource.LevelStorageAccess levelStorageAccess, LevelSummary levelSummary, Dynamic<?> dynamic, Runnable runnable) {
        if (!levelSummary.isCompatible()) {
            levelStorageAccess.safeClose();
            this.minecraft.setScreen(new AlertScreen(runnable, Component.translatable("selectWorld.incompatible.title").withColor(-65536), Component.translatable("selectWorld.incompatible.description", levelSummary.getWorldVersionName())));
            return;
        }
        ...
        if (backupStatus.shouldBackup()) {
            ...
            MutableComponent mutableComponent2 = Component.translatable(string2, levelSummary.getWorldVersionName(), SharedConstants.getCurrentVersion().name());
            ...
            }, mutableComponent, mutableComponent2, false));
        } else {
            this.openWorldLoadLevelStem(levelStorageAccess, dynamic, false, runnable);
        }
    }
    ...
    private void openWorldCheckWorldStemCompatibility(LevelStorageSource.LevelStorageAccess levelStorageAccess, WorldStem worldStem, PackRepository packRepository, Runnable runnable) {
        ...
        boolean bl2 = worldData.worldGenOptions().isOldCustomizedWorld();
        boolean bl3 = bl = worldData.worldGenSettingsLifecycle() != Lifecycle.stable();
        if (bl2 || bl) {
            this.askForBackup(levelStorageAccess, bl2, () -> this.openWorldLoadBundledResourcePack(levelStorageAccess, worldStem, packRepository, runnable), () -> {
                worldStem.close();
                levelStorageAccess.safeClose();
                runnable.run();
            });
    ...
    private void openWorldLoadBundledResourcePack(LevelStorageSource.LevelStorageAccess levelStorageAccess, WorldStem worldStem, PackRepository packRepository, Runnable runnable) {
        ...
        }, (Executor)this.minecraft)).thenAcceptAsync(bl -> {
            if (bl.booleanValue()) {
