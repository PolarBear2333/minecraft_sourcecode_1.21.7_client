...
extends EntityModel<IronGolemRenderState> {
    ...
    public void setupAnim(IronGolemRenderState ironGolemRenderState) {
        ...
        float f2 = ironGolemRenderState.walkAnimationSpeed;
        float f3 = ironGolemRenderState.walkAnimationPos;
        if (f > 0.0f) {
            this.rightArm.xRot = -2.0f + 1.5f * Mth.triangleWave(f, 10.0f);
            this.leftArm.xRot = -2.0f + 1.5f * Mth.triangleWave(f, 10.0f);
        } else {
            int n = ironGolemRenderState.offerFlowerTick;
            if (n > 0) {
                this.rightArm.xRot = -0.8f + 0.025f * Mth.triangleWave(n, 70.0f);
                this.leftArm.xRot = 0.0f;
            } else {
                this.rightArm.xRot = (-0.2f + 1.5f * Mth.triangleWave(f3, 13.0f)) * f2;
                this.leftArm.xRot = (-0.2f - 1.5f * Mth.triangleWave(f3, 13.0f)) * f2;
            }
        }
        this.head.yRot = ironGolemRenderState.yRot * ((float)Math.PI / 180);
        this.head.xRot = ironGolemRenderState.xRot * ((float)Math.PI / 180);
        this.rightLeg.xRot = -1.5f * Mth.triangleWave(f3, 13.0f) * f2;
        this.leftLeg.xRot = 1.5f * Mth.triangleWave(f3, 13.0f) * f2;
