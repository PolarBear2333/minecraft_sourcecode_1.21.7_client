...
extends OptionalDispenseItemBehavior {
    ...
    protected ItemStack execute(BlockSource blockSource, ItemStack itemStack) {
        ...
        if (!serverLevel.isClientSide()) {
            ...
            this.setSuccess(ShearsDispenseItemBehavior.tryShearBeehive(serverLevel, blockPos) || ShearsDispenseItemBehavior.tryShearEntity(serverLevel, blockPos, itemStack));
            if (this.isSuccess()) {
    ...
    private static boolean tryShearBeehive(ServerLevel serverLevel, BlockPos blockPos) {
        ...
        if (blockState.is(BlockTags.BEEHIVES, blockStateBase -> blockStateBase.hasProperty(BeehiveBlock.HONEY_LEVEL) && blockStateBase.getBlock() instanceof BeehiveBlock) && (n = blockState.getValue(BeehiveBlock.HONEY_LEVEL).intValue()) >= 5) {
