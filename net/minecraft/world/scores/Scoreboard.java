...
public class Scoreboard {
    ...
    public Collection<PlayerScoreEntry> listPlayerScores(Objective objective) {
        ArrayList<PlayerScoreEntry> arrayList = new ArrayList<PlayerScoreEntry>();
        this.playerScores.forEach((string, playerScores) -> {
            Score score = playerScores.get(objective);
            if (score != null) {
                arrayList.add(new PlayerScoreEntry((String)string, score.value(), score.display(), score.numberFormat()));
            }
        });
        return arrayList;
    }
    ...
    protected List<PackedScore> packPlayerScores() {
        return this.playerScores.entrySet().stream().flatMap(entry2 -> {
            ...
            return ((PlayerScores)entry2.getValue()).listRawScores().entrySet().stream().map(entry -> new PackedScore(string, ((Objective)entry.getKey()).getName(), (Score)entry.getValue()));
        }).toList();
    }
    ...
    protected void loadPlayerScore(PackedScore packedScore) {
        ...
        this.getOrCreatePlayerInfo(packedScore.owner).setScore(objective, packedScore.score);
    }
    ...
    extends Record {
        ...
        public PackedScore(String string, String string2, Score score) {
            this.owner = string;
