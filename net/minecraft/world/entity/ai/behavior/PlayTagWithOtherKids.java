...
public class PlayTagWithOtherKids {
    ...
    public static BehaviorControl<PathfinderMob> create() {
        return BehaviorBuilder.create(instance -> instance.group(instance.present(MemoryModuleType.VISIBLE_VILLAGER_BABIES), instance.absent(MemoryModuleType.WALK_TARGET), instance.registered(MemoryModuleType.LOOK_TARGET), instance.registered(MemoryModuleType.INTERACTION_TARGET)).apply((Applicative)instance, (memoryAccessor, memoryAccessor2, memoryAccessor3, memoryAccessor4) -> (serverLevel, pathfinderMob, l) -> {
    ...
    private static Map<LivingEntity, Integer> checkHowManyChasersEachFriendHas(List<LivingEntity> list) {
        ...
        list.stream().filter(PlayTagWithOtherKids::isChasingSomeone).forEach(livingEntity2 -> hashMap.compute(PlayTagWithOtherKids.whoAreYouChasing(livingEntity2), (livingEntity, n) -> n == null ? 1 : n + 1));
        return hashMap;
    }
    ...
    private static boolean isFriendChasingMe(LivingEntity livingEntity, LivingEntity livingEntity3) {
