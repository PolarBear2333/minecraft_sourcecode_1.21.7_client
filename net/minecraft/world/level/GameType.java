...
public enum GameType implements StringRepresentable
{
    ...
    @Nullable
    public static GameType byNullableId(int n) {
        if (n == -1) {
            return null;
        }
        return GameType.byId(n);
    }

    public static boolean isValidId(int n) {
        return Arrays.stream(GameType.values()).anyMatch(gameType -> gameType.id == n);
    }
