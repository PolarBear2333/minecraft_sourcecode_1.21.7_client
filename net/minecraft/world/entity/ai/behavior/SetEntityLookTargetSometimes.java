...
public class SetEntityLookTargetSometimes {
    ...
    private static BehaviorControl<LivingEntity> create(float f, UniformInt uniformInt, Predicate<LivingEntity> predicate) {
        ...
        return BehaviorBuilder.create(instance -> instance.group(instance.absent(MemoryModuleType.LOOK_TARGET), instance.present(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES)).apply((Applicative)instance, (memoryAccessor, memoryAccessor2) -> (serverLevel, livingEntity, l) -> {
            Optional<LivingEntity> optional = ((NearestVisibleLivingEntities)instance.get(memoryAccessor2)).findClosest(predicate.and(livingEntity2 -> livingEntity2.distanceToSqr(livingEntity) <= (double)f2));
            if (optional.isEmpty()) {
                return false;
            }
