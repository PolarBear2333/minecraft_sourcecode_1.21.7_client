...
public class ScheduleCommand {
    ...
    private static int schedule(CommandSourceStack commandSourceStack, Pair<ResourceLocation, Either<CommandFunction<CommandSourceStack>, Collection<CommandFunction<CommandSourceStack>>>> pair, int n, boolean bl) throws CommandSyntaxException {
        ...
        if (optional.isPresent()) {
            ...
            commandSourceStack.sendSuccess(() -> Component.translatable("commands.schedule.created.function", Component.translationArg(resourceLocation), n, l), true);
        } else {
            String string = "#" + String.valueOf(resourceLocation);
            if (bl) {
                timerQueue.remove(string);
            }
            timerQueue.schedule(string, l, new FunctionTagCallback(resourceLocation));
            commandSourceStack.sendSuccess(() -> Component.translatable("commands.schedule.created.tag", Component.translationArg(resourceLocation), n, l), true);
        }
