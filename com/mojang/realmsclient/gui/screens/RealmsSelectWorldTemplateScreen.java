...
extends RealmsScreen {
    ...
    private void renderMultilineMessage(GuiGraphics guiGraphics, int n, int n2, List<TextRenderingUtils.Line> list) {
        for (int i = 0; i < list.size(); ++i) {
            ...
            for (TextRenderingUtils.LineSegment lineSegment2 : line.segments) {
                int n6 = lineSegment2.isLink() ? -13408581 : -1;
                String string = lineSegment2.renderedText();
                guiGraphics.drawString(this.font, string, n5, n3, n6);
                int n7 = n5 + this.font.width(string);
                if (lineSegment2.isLink() && n > n5 && n < n7 && n2 > n3 - 3 && n2 < n3 + 8) {
                    guiGraphics.setTooltipForNextFrame(Component.literal(lineSegment2.getLinkUrl()), n, n2);
    ...
    extends ObjectSelectionList.Entry<Entry> {
        ...
        public void render(GuiGraphics guiGraphics, int n, int n2, int n3, int n4, int n5, int n6, int n7, boolean bl, float f) {
            guiGraphics.blit(RenderPipelines.GUI_TEXTURED, RealmsTextureManager.worldTemplate(this.template.id, this.template.image), n3 + 1, n2 + 1 + 1, 0.0f, 0.0f, 38, 38, 38, 38);
            guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, SLOT_FRAME_SPRITE, n3, n2 + 1, 40, 40);
            int n8 = 5;
            ...
            guiGraphics.drawString(RealmsSelectWorldTemplateScreen.this.font, this.template.version, n3 + n4 - n9 - 5, n11, -9671572);
            ...
            if (!this.template.recommendedPlayers.isBlank()) {
                guiGraphics.drawString(RealmsSelectWorldTemplateScreen.this.font, this.template.recommendedPlayers, n10, n2 + n5 - ((RealmsSelectWorldTemplateScreen)RealmsSelectWorldTemplateScreen.this).font.lineHeight / 2 - 5, -11776948);
            }
        }
