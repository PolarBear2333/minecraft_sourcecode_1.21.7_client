...
public record TeleportRandomlyConsumeEffect(float diameter) implements ConsumeEffect
{
    ...
    public boolean apply(Level level, ItemStack itemStack, LivingEntity livingEntity) {
        ...
        for (int i = 0; i < 16; ++i) {
            ...
            double d2 = Mth.clamp(livingEntity.getY() + (livingEntity.getRandom().nextDouble() - 0.5) * (double)this.diameter, (double)level.getMinY(), (double)(level.getMinY() + ((ServerLevel)level).getLogicalHeight() - 1));
            double d3 = livingEntity.getZ() + (livingEntity.getRandom().nextDouble() - 0.5) * (double)this.diameter;
