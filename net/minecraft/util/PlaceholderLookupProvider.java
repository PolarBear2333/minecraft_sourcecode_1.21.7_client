...
implements HolderGetter.Provider {
    ...
    public RegistryContextSwapper createSwapper() {
        return new RegistryContextSwapper(){
            ...
            @Override
            public <T> DataResult<T> swapTo(Codec<T> codec, T t, HolderLookup.Provider provider) {
                return codec.encodeStart(PlaceholderLookupProvider.this.createSerializationContext(JavaOps.INSTANCE), t).flatMap(object -> codec.parse(provider.createSerializationContext(JavaOps.INSTANCE), object));
            }
        };
    }
